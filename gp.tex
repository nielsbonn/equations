...
{\it Remarks. } 
\begin{itemize}
\item If we relax (\ref{eq:ass2-N}) to the weaker condition $\| P_N W (\sqrt{N} \varphi) T(k_0) \psi \| \geq C N^{-\alpha}$, for some $1/4 \leq \alpha < 1/2$, the proof below still implies the convergence $\gamma^{(1)}_{N,t} \to |\varphi_t \rangle \langle \varphi_t|$ but only with the slower rate $N^{-1/2+ \alpha}$. 
\item The assumption (\ref{eq:ass2-N}) and its weaker versions mentioned in the previous remark 
...
\bibitem{FKS}
J. Fr\"ohlich, A. Knowles, S. Schwarz.
