...
and its hermitian conjugate. To control (\ref{eq:exc-K}), we use that by definition (\ref{eq:kt})
...
where $\ad^0_B (A) = A$ and $\ad^n_B (A) = [B,\ad^{n-1}_B (A)]$ for all $n \geq 1$. Since the operators $B$ and $\dot{B}$ are unbounded, some care is needed when dealing with this formula. We will see however, that the series is absolute convergent on the domain of the number of particles operator $D(\cN)$. Since $D(\cN)$ is invariant w.r.t. the fluctuation dynamics $\cU (t,s)$ (this can be proved using Lemma \ref{lm:TNT} and the methods used to prove Prop.\ \ref{prop:apri}), (\ref{eq:BCH}) can be used to compute the expectation of $(\partial_t T^*) T$ in the state $\cU (t;0) \psi$, for any $\psi \in D(\cN)$. 
