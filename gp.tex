...
Then, the solution $\psi_{N,t}$ of the Schr\"odinger equation (\ref{eq:schr}) still exhibits complete Bose-Einstein condensation, in the sense that the reduced one-particle density $\gamma_{N,t}^{(1)}$ associated with $\psi_{N,t}$ satisfies at any fixed $t \in \Rbb$ that
...
to prove (\ref{eq:conv-GP}) relies on BBGKY hierarchy techniques first developed to understand the mean field limit of many-body quantum dynamics. This regime is achieved when considering the time-evolution 
...
The first rigorous derivation of (\ref{eq:hartree-0}) was obtained in \cite{S} for bounded interaction potential, i.e.\ under the assumption $\| V \|_\infty < \infty$.
The $k$-particle reduced density matrix $\gamma^{(k)}_{N,t}$ is defined by taking the partial trace over all but $k$ particles, i.\,e.\ $\gamma^{(k)}_{N,t}$ is the operator on $L^2(\Rbb^{3k})$ with the integral kernel
...
where $\mathbf{x} = (x_1,\ldots,x_k) \in \Rbb^{3k}$; the normalization is $\tr \; \gamma^{(k)}_{N,t} = 1$. The basic idea in \cite{S} was to study directly the time-evolution of the family of reduced densities $\gamma^{(k)}_{N,t}$, $k =1 ,2, \dots , N$. Rewriting the Schr\"odinger equation \eqref{eq:mf-schr} in terms of the density matrix $\gamma^{(N)}_{N,t} = \lvert \psi_{N,t}\rangle\langle \psi_{N,t}\rvert$ as
$i\partial_t \gamma^{(N)}_{N,t} = [H^{\text{mf}}_N, \gamma^{(N)}_{N,t}]$
and taking the partial trace, one derives a hierarchy of $N$ coupled equations, known as the Bogoliubov–Born–Green–Kirkwood–Yvon hierarchy:
\begin{equation}
\label{eq:bbgky_hi}
...
\end{split}     
\end{equation}
for $k=1,\ldots N$, with the convention that $\gamma^{(k)}_{N,t} = 0$ for $k>N$, and with $\tr_{k+1}$ the partial trace over the $(k+1)$-th factor of the tensor product.
...
potentials with Coulomb singularities in \cite{EY} (and in \cite{ES}, for bosons with relativistic dispersions).
...
Plugging this ansatz into (\ref{eq:BBGKY1}) the hierarchy is cut off\footnote{It is a common technique in physics to cut of the BBGKY hierarchy after the first one or two reduced densities to heuristically obtain effective equations. Notice however that it is also possible to truncate the BBGKY hierarchy at the $K$-particle reduced density and remove the truncation in the limit, which is a technique useful also for rigorous purposes \cite{ChenPav}.} and a closed equation obtained, from which we obtain a self-consistent equation for $\varphi_t$, namely
...
\item[(iv)] To simplify a little bit the computations, we did not include an external potential in the Hamiltonian (\ref{eq:cHN}) generating the evolution on the Fock space. In contrast to \cite{ESY1,ESY2,ESY3,ESY4}, the approach presented in this paper can be extended with no additional complication to Hamilton operators with external potential. For comparison, notice the works \cite{XChen1,XChen2} on the addition of external potentials. The inclusion of external potentials is important to describe experiments where the evolution of the condensate is observed after tuning the magnetic traps, rather than switching them off. 
...
for every $\lambda \in \bR$. To prove (\ref{eq:NTN}), we observe that for all $\psi \in \fock$
...
Assuming that $n$ is odd, (\ref{eq:NTN}) implies that 
...
Define the time-dependent scalar
...
\end{split} \end{equation}
(In $C_N(t)$ we collect all summands of the generator which do not contain any creation or annihilation operators. The precise form of these terms does not matter for our analysis, since they simply correspond to an overall phase. Notice however that $C_N(t)$ is of order $N$.) Let
...
for a constant $D>0$ (uniform in $N$). Let $\varphi \in H^4 (\bR^3)$, and let $\varphi_t^{(N)}$ be the solution of the modified Gross-Pitaevskii equation (\ref{eq:mod-GP}) with initial data $\varphi$. Let $\cU (t;s)$ be the fluctuation dynamics defined in (\ref{eq:cU}). Then there exist constants $C,c_1,c_2 > 0$ such that
...
(t)$ appearing in (\ref{eq:thmL-2}) is a scalar and hence does not
...
%    \phi(f) \K \phi(f) & \le C_1 \big( \| f \|_{L^2}^2 \K (\N+1) + \| \nabla f \|_{L^2}^2 (\N + 1) \big). \label{fKf} \tag{iv}
  \end{align}
for all $t \in \bR$. Furthermore, the same inequalities hold with $T$ replaced by $T^*$.
\end{lemma}
...
\begin{proof}[Proof of Proposition \ref{prop:apri}]
  From Lemma \ref{lm:TNT} (with $T$ replaced by $T^*$) and Cauchy-Schwarz, we find
\begin{equation}\label{ep4-0}   \langle \psi , \cU^* (t;0) \cN^2 \cU (t;0) \psi \rangle \leq C \left( \langle \psi , \cU^* (t;0) T^* (k_t) \cN^2 T (k_t) \cU (t;0) \psi \rangle +\langle \psi,\psi\rangle\right). \end{equation}
...
(We will also use this formula with $\varphi_t^{(N)}$ replaced by $-\varphi$. Note that $W(-\varphi)=W^*(\varphi)$ and $\phi(-\varphi)=-\phi(\varphi)$.) In the first term on the r.h.s.\ of (\ref{ep4}), we use now the fact that $\cN$ commutes with $\cH_N$. In the second term, on the other hand, we move the factor $\phi (\varphi_t^{(N)})$ back to the left of the Weyl operator $W (\sqrt{N} \varphi_t^{(N)})$, using that \[ W^* (\sqrt{N} \varphi_t^{(N)}) \phi (\varphi_t^{(N)}) = \left(\phi (\varphi_t^{(N)}) + 2 \sqrt{N} \right) \, W^* (\sqrt{N} \varphi_t^{(N)})\,. \] We conclude that
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We are now ready to show Theorem \ref{thm:N}. The basic idea is to apply the Gr\"onwall lemma. Unfortunately we can not control $\frac{d}{dt} \langle \psi, \wt{\cU}^* (t;0) \cN \wt{\cU} (t;0) \psi \rangle$ with $\langle \psi, \wt{\cU}^* (t;0) (\cN+1) \wt{\cU} (t;0) \psi \rangle$; however, we can bound it using $\tilde\Lcal_N(t)+\Ncal+1$. Similarly we can bound the derivative of the expectation of $\tilde\Lcal_N(t)$ using $\tilde\Lcal_N(t)+\Ncal+1$. Consequently, we can bound the derivative of the expectation value of $\tilde\Lcal_N(t)+\Ncal$ using $\tilde\Lcal_N(t)+\Ncal+1$, and thus apply the Gr\"onwall lemma. We then use a lower bound for $\tilde\Lcal_N(t)$ to derive a bound for the expectation of $\Ncal$. The precise calculation looks a bit more complicated since we have to keep track of the constants (and the exponential factors).
...
The second bound in (\ref{eq:T2T-cl}) follows similarly, using the fact that the commutator of $\cN$ with the terms on the r.h.s.\ of (\ref{eq:T2T}) leaves their form unchanged (apart from the scalar (not containing any creation/annihilation operators) terms and the quadratic terms with one creation and one annihilation operators, whose contribution to the commutator $[\cN , \cE_2 (t)]$ vanishes).  
...
Then $T = \exp (B)$. Using the BCH formula \eqref{eq:baker}, we find%(for $\ad^0_B (\dot B) = \dot B$ and $\ad^n_B (\dot B) = [B,\ad^{n-1}_B (\dot B)]$)
...
\\ &+ \cE (t) 
\end{split} \end{equation}
where the time-dependent scalar $C_N (t)$ is defined in (\ref{eq:CNt}) and the error $\cE (t)$ is such that, for every $\delta > 0$ there exists $C_\delta > 0$ with
...
for a constant $D>0$ (uniform in $N$). Let $P_N$ denote the projection onto the $N$-particle sector of the Fock space and assume that 
