...
\item[(v)] We expect that Theorem \ref{thm:main} and the method used in its proof can also be applied to deduce the convergence towards the Gross-Pitaevskii dynamics for initial data with a fixed number $N$ of particles. In fact one immediately gets a convergence similar to (\ref{eq:mt}) (but with slower rate) for initial $N$-particles states of the form $P_N W(\sqrt{N} \varphi) T(k_0) \psi$\done, for $\psi \in \cF$
satisfying (\ref{eq:ass-thm1}), provided $\| P_N W(\sqrt{N} \varphi) T(k_0) \psi \| \gg N^{-1/2}$\done for large $N$. Here $P_N$ is the orthogonal projection onto the $N$-particle sector of $\cF$. It is not yet clear to us which $N$-particles states can be written this way. We will come back to this problem in a separate work.
\end{itemize}
...
\newblock {\em J. Amer. Math. Soc.}, 22:1099--1156,
  2009.
\iffalse
\bibitem{ESY0}
