...
Concerning the time dependence, notice also that Theorem~\ref{thm:main} in particular implies that for any fixed time interval, for $N$ large enough the error becomes arbitrarily small, uniformly on the whole interval. This should be compared to the earlier results cited in \eqref{eq:conv-GP}, which do not provide any uniform in time convergence, not even on finite intervals.
