...
where $\mathbf{x} = (x_1,\ldots,x_k) \in \Rbb^{3k}$; the normalization is chosen so that $\tr \; \gamma^{(k)}_{N,t} = 1$. {F}rom the Schr\"odinger equation \eqref{eq:mf-schr}, it is possible to derive a hierarchy of $N$ coupled equation, known as the BBGKY hierarchy (for Bogoliubov, Born, Green, Kirkwood and Yvon), given by 
\begin{equation}
\label{eq:bbgky_hi}
...
\end{equation}
{F}rom (\ref{eq:8pia}), we have $N^2 (N-1) V(Nx) f(Nx) \simeq N^3 V(Nx) f(Nx) \simeq 8\pi a_0 \delta (x)$ as $N \to \infty$; therefore, in this limit, $\varphi_t$ must be a solution of the Gross-Pitaevskii equation (\ref{eq:GP-0}). We observe here that the presence of the factor $f(N.)$, describing the correlations among the particles, is crucial in this argument to understand the emergence of the scattering length in  (\ref{eq:GP-0}). While in the mean field limit it is possible to close the BBGKY hierarchy assuming the factorization $\gamma^{(2)}_{N,t} \simeq \gamma^{(1)}_{N,t} \otimes \gamma^{(1)}_{N,t}$, here is is important to take into account the singular correlation structure
...
\end{split} \end{equation}
of the dynamics (\ref{eq:cU-0}). We will prove in Section \ref{sec:gen-fd} that the term 
$\left( i \partial_t T^* (k_t) \right) T(k_t)$ is harmless (see Proposition~\ref{prp:dTT}). As for the 
first line on the r.h.s. of (\ref{eq:Ln}), it will still contain the large contribution (\ref{eq:lin-N}). In this case, however, this dangerous term will be compensated by a contribution arising from the conjugation of the cubic part of $W^* (\sqrt{N} \varphi_t^{(N)}) \cH_N W(\sqrt{N} \varphi_t^{(N)})$ with the Bogoliubov transformation $T(k_t)$. In fact, according to (\ref{eq:TT}), conjugation with $T(k_t)$ will produce terms with creation operators standing to the right of annihilation operators; restoring 
normal order will then generate linear terms, canceling (\ref{eq:lin-N}). Other important cancellations will emerge between the quadratic and the quartic part of $W^* (\sqrt{N} \varphi_t^{(N)}) \cH_N W(\sqrt{N} \varphi_t^{(N)})$ (again, after conjugation with the Bogoliubov transformation $T(k_t)$; see Section \ref{sec:gen-fd} for the details. The control of the growth of the number of particles w.r.t. (\ref{eq:cU-0}) will imply convergence of the one-particle reduced density matrix associated with the fully evolved Fock state $e^{-i \cH_N t} W(\sqrt{N} \varphi_t) T(k_t) \psi$ towards the orthogonal projection onto the solution of the Gross-Pitaevskii equation (\ref{eq:GP-0}) as $N \to \infty$, with a bound on the rate of the convergence.
...
$\| \varphi \|_2 = 1$.
\begin{itemize}
\item[(i)] Well-posedness. There exist unique global solutions $\varphi_. , \varphi_.^{(N)} \in C(\bR ; H^1 (\bR^3))$ of the Gross-Pitaevskii equation (\ref{eq:GP}) and, respectively, of the modified Gross-Pitaevskii equation (\ref{eq:mod-GP}) with initial data $\varphi$. These solutions are such that $\| \varphi_t \|_2 = \| \varphi_t^{(N)} \|_2 = 1$ for all $t \in \bR$. Moreover, there exists a constant $C > 0$ such that
...
\[ \begin{split} 
\langle \psi , \cU^* (t;0) &T^* (k_t) \cN^2  T(k_t) \cU (t;0) \psi \rangle \\ \leq \; & \| \cN \, T (k_t) \cU (t;0) \psi \|\\ & \hspace{1cm} \times  \left(  \| \cN \, T (k_0) \psi \| + \sqrt{N} \| \phi (\varphi) \, T (k_0) \psi \| + \sqrt{N} \|  \phi (\varphi_t^{(N)}) T (k_t) \cU (t;0)  \psi \| \right) \\ \leq \; & \frac{1}{2} \| \cN \, T (k_t) \cU (t;0) \psi \|^2 \\ &+ C \left( \|
...
%N \left[ \| \nabla \varphi_t^{(N)} \|_2^2 + \frac{1}{2} \int dx \, (N^3 V (N .) * |\varphi_t^{(N)}|^2) (x) |\varphi_t^{(N)} (x)|^2 \right] \\
N \int \di x\left( N^3 V(N.)\Big(\frac{1}{2}-f(N.)\Big)\ast \lvert \ph\rvert^2 \right)(x) \lvert \ph(x)\rvert^2\\
&+ \sqrt{N}  \left[  a^*\big( (N^3
w(N.) V (N.) *| \varphi_t^{(N)}|^2) \varphi_t^{(N)} \big)
% \right. \\ & \quad \qquad \left. 
+ a\big( (N^3 w (N.)V (N.) *| \varphi_t^{(N)}|^2) \varphi_t^{(N)} \big) \right] \\
&+ \left[ \int dx \, \nabla_x a_x^* \nabla_x a_x + \int dx \, (N^3 V (N.) * |\varphi_t^{(N)}|^2) (x) a_x^* a_x \right. \\ 
&\hspace{.5cm}+ \int dx dy N^3 V(N (x-y)) \varphi_t^{(N)} (x) \overline{\varphi}_t^{(N)} (y) a_x^* a_y \\
&\hspace{.5cm}+ \left.  \frac{1}{2} \int dx dy N^3 V(N (x-y)) \left( \varphi_t^{(N)} (x) \varphi_t^{(N)} (y) a_x^* a_y^* + 
\overline{\varphi}_t^{(N)} (x) \overline{\varphi}_t^{(N)} (y) a_x a_y \right) \right] \\
...
\int dx dy &N^3 V(N (x-y)) |\varphi_t^{(N)} (x)| |\varphi_t^{(N)} (y)|  \| a^\sharp (j_{1,x}) \psi \| \, \| a^\sharp (j_{2,y}) \psi \| \\ \leq \; & \alpha \int dx dy N^3 V(N (x-y)) |\varphi^{(N)}_t (x)|^2 \| j_{2,y} \|_2^2 \| (\cN+1)^{1/2} \psi \|^2 \\ &+ \alpha^{-1}
 ...
 \leq \; & \| N^3 V(N.) * |\varphi_t^{(N)}|^2 \|_\infty  \left( \alpha \| j_{1} \|^2_2 + \alpha^{-1} \| j_2 \|_{2}^2 \right) \, \| (\cN+1)^{1/2} \psi \|^2.
 \end{split} \]
...
where $\| J \|$ denotes the operator norm of $J$. {F}rom Lemma \ref{lm:TNT} and recalling the definition 
...
Rigorous derivation of the cubic NLS in dimension one. {\it J. Stat. Phys.} {\bf 127} (2007),
 no. 6, 1193--1220.
...
\bibitem{KSS}
K. Kirkpatrick, B. Schlein, G. Staffilani: Derivation of the two dimensional nonlinear
...
\bibitem{LNSS}
M. Lewin, P. T. Nam, S. Serfaty, and J. P. Solovej. Bogoliubov spectrum of interacting {B}ose gases. Preprint arXiv:1211.2778.
...
\bibitem{P}
P.~{Pickl}.
\newblock {Derivation of the time dependent {G}ross {P}itaevskii equation with
