...
\bd
\norm{N^3 V(N(\cdot))w(N(\cdot))^2}_{L^1} = \int \di z V(z) w(z)^2 \leq \int \di z V(z) = b_0.
\ed
Furthermore, by Sobolev inequality and using the Lemma \ref{lm:phdotregularity} on regularity of $\phdot$
\bd
\norm{\lvert \phdot\rvert^2}_{L^2} = \norm{\phdot}_{L^4}^2 \leq C \norm{\phdot}_{H^1}^2 \leq C(\norm{\ph}_{H^2}^2 + \norm{\ph}_{H^3}^2)^2 \leq C \norm{\ph}_{H^3}^4.
\ed
Also by a Sobolev inequality $\norm{\lvert \ph\rvert^2}_{L^2} \leq C$.
...
\bd
\lvert (f_1 \cdots f_N)(x,y)\rvert \leq \left( \int \di z \lvert f_1(x,z)\rvert^2 \right)^{1/2} \left( \int \di z \lvert f_n(x,y)\rvert^2\right)^{1/2} \norm{f_2} \cdots \norm{f_{n-1}}.
\ed
We now apply this to
\begin{align*}
& \lvert \dot r(x,y) \rvert = \lvert \sum_{n=1}^\infty \frac{1}{(2n+1)!} \partial_t \left(k(\cc k k)^n \right)(x,y)\rvert \\
& \leq \sum_{n=1}^\infty \frac{1}{(2n+1)!} \left\lvert\left( \dot k(\cc k k)^n + \sum_{j=1}^{n-1} k(\cc k k)^{j-1}(\cc{\dot k}k + \cc k \dot k)(\cc k k)^{n-j} + k (\cc k k)^{n-1} \cc{\dot k} k + k(\cc{k}k)^{n-1}\cc{k}\dot k \right)(x,y)\right\rvert \\
...
\end{align*}
As $\norm{k}$ can be bounded by a constant $C$ independent of $N$ and $t$, so can the series appearing here (clearly, the series are summable). Using Hardy's inequality, one can easily show that
\bd
...
\ed
The estimate for $\lvert \dot r(x,y)\rvert$ is now used to conclude
\begin{align*}
...
For \eqref{eq:kk1} and \eqref{eq:kk3} it is a non-trivial task to find an estimate because $\nabla w_N$ has a stronger singularity. We will show the proof for \eqref{eq:kk1}; the proof for \eqref{eq:kk3} works similarly and yields the bound $C \norm{\nabla \phdot}_{L^2}^2$.\\
\underline{Bound for \eqref{eq:kk1}}:\\
\begin{align*}
\eqref{eq:kk1} = & \int \di x \di y \left\lvert \int N \nabla w_N(x-z) \phdot(x) \lvert \ph(z)\rvert^2 \cc{\ph(y)} N w_N(z-y) \di z \right\rvert^2 \\
& \leq C \int \di x \di y \left\lvert \int \di z \frac{1}{\lvert x-z\rvert^2} \phdot(x) \lvert \ph(z)\rvert^2 \cc{\ph(y)} \frac{1}{\lvert z-y\rvert} \right\rvert^2 \\
...
\end{align*}
The integral w.\,r.\ to $y$ can be bounded by $C \norm{\nabla \ph}_{L^2}^2 \leq C$ using Hoelder's inequality and then Hardy's inequality. So we get
\bd
...
$\norm{\nabla_1 \dot p}$, $\norm{\nabla_1 \dot r}$, $\norm{\dot s}$, $\norm{\dot p}$, $\sup_x \norm{\dot p_x}$, $\sup_x \norm{\dot s_x}$, $\int \di x\di y V_N(x-y) \lvert \dot r(x,y)\rvert^2$, $\int \di x \di y V_N(x-y) \lvert \dot s(x,y)\rvert^2/N$ and different norms of $\phdot$. These quantities are then estimated using the Lemmata \ref{lem:dottedests1}, \ref{lem:dottedests2} and \ref{lem:dottedests3}.

(Remarkt: In $\partial_t( T^\ast_t \Lcal_N^{(0)}(t) T_t)$ there are four times as many terms as in $T^\ast_t \Lcal_N^{(0)}(t) T_t$ because of the product rule, but many of them are easy to estimate because $\partial_t a^\ast(c_x) = a^\ast(\dot p_x)$ is regular while $a^\ast(c_x)$ is singular.)

We arrive at
\bd
\lvert \scal{\psi}{\dot \Lcal_N(t)\psi} \rvert \leq C \norm{\ph}_{H^4}^8 \scal{\psi}{\left(\frac{\Ncal^2}{N}+\Ncal+1+\Kcal+\tilV \right)\psi}.
\ed
(Again, we made no effort to optimize the appearance of $H^n$ norms.)
\end{proof}
...
Recall also that
\[
  W = W(\sqrt{N} \varphi) \qquad \text{and} \qquad T = T(k_0).
\]
...
\end{cor}
\begin{proof}
This is evident from Lemma \ref{l:ap} \ref{l:ap1} and Proposition \ref{p:TNT}.
\end{proof}
...
\begin{proof}[Proof of Proposition \ref{p:reg1}]
  ...
  \marginpar{in the following equations, is it really $\varphi^2$ or maybe
  $\lvert \varphi\rvert^2$? Ans: $\| z^2 \| = \| |z|^2 \|$}
...
\begin{proof}[Proof of Corollary \ref{c:reg1}]
