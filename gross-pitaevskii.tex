...
\[
  \Hcal_N^{(n)} = \sum_{j=1}^n (-\Delta_{x_j}) + \sum_{i<j}^n V_N(x_i - x_j) = \sum_{j=1}^n (-\Delta_{x_j}) + \sum_{j=1}^n \sum_{i=1}^{j-1} V_N(x_i - x_j),
\]
where $V_N(x) = N^2 V(Nx)$ with $V \in C_c^\infty(\R^3)$. We assume that $V$ is non-negative and spherically symmetric. By definition, the
Hamiltonian $\Hcal_N$ leaves sectors of $\fock$ with a fixed number of
particles invariant and $[\Hcal_N,\Ncal] = 0$.
...
\[
  W(f) = e^{a^*(f) - a(f)}.
\]
States $W(f)\Omega$ are called coherent states.
