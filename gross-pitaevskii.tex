...
\begin{prp}
  ...
  \begin{align}
    ...
    T^* \K T & \apprle C \K + \| \nabla_1 s \|_{L^2}^2, \label{TKT} \tag{iii}
    \\
    T^* \K \N T & \apprle \frac{C}{N} \N T^* \K T \N + C N T^* \K T + C^2 \K +
    C \| \nabla_1 s \|_{L^2}^2, \label{TKNT} \tag{iv}
  \end{align}
...
\begin{proof}[Proof of Proposition \ref{p:TNT}]
  ...
  sqrt?}
  Thus, applying part (i) and Cauchy-Schwarz inequality, and using the
  ...
  \begin{align*}
    ...
    & \le 25C \int dx \, ( \langle a_x^* a_x \psi, \N \psi \rangle + \|
    a(p_x) \N^{1/2} \psi \|^2 + \| a^*(s_x) (\N+2)^{1/2} \psi \|^2 ) + C
    \langle \psi, (\N+1) \psi \rangle \\
    & \apprle C^2 \langle \psi, (\N+2)^2 \psi \rangle \apprle C^2 \langle \psi,
    (\N+1)^2 \psi \rangle,
  \end{align*}
  where $C = 1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2$.


  (iii) Similarly as in the proof of (i), write
  \[
  ...
  \begin{align*}
    ...
    a^*(\nabla_x s_x) \psi \|^2 \\
    & \apprle \langle \psi, \K \psi \rangle + \int dx \, \| p_x \|_{L^2}^2
    \langle \psi, \K \psi \rangle + \int dx \, \langle \psi, a(\nabla_x s_x)
    a^*(\nabla_x s_x) \psi \rangle \\
    & = \langle \psi, \K \psi \rangle + \| \nabla_1 p \|_{L^2}^2 \langle \psi,
    \K \psi \rangle + \int dx \, \| a(\nabla_x s_x) \psi \|^2 + \| \nabla_1 s
    \|_{L^2}^2 \\
    & \le (1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2) \langle \psi, \K \psi
    \rangle + \| \nabla_1 s \|_{L^2}^2.
  \end{align*}


  (iv) First observe that
  \begin{align*}
    \K \N & = \int dx dy \, \nabla_x a_x^* \nabla_x a_x a_y^* a_y \\
