...
that is
\[
\Tr \left\lvert \gamma_{N}^{(1)} - \project{\varphi} \right\rvert \leq \frac{C}{N}e^{Kt}.
\]
Notice that formally for $U = N V_N \to b_0
...
is $8 \pi a_0$ and not $b_0$. (The result in that
...
\ee
For $\psi$ in the $N$-particle subspace of $\fock$ this definition is equivalent to the definition in \eqr{red_density}.
