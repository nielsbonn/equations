...
Our method is based on using the Bogoliubov transformation to implement a short-scale structure on coherent initial states, which thus become squeezed coherent states (?). The Gross-Pitaevskii limit can then be treated as a mean-field limit. We obtain explicit estimates on the rate of convergence of $k$-particle reduced density matrices.
\end{abstract}
...
The method of our proof is based on the study of the evolution of coherent states. Rodniaski and Schlein \cite{RS2009} show that for solutions to the Schr\"odinger equation with Hamiltonian
\[
...
Therefore we compare the evolution of coherent states with the evolution given by the modified Hartree equation
\be{modHartreeEqu}
i \partial_t \ph = -\Delta \ph + \left(N f_N V_N \ast \lvert \ph\rvert^2 \right) \ph,
\ee
which formally has the correct limiting equation.
Using the Bogoliubov transformation we can then explicitly exhibit cancellations in the generator of fluctuation dynamics around the evolution given by the modified Hartree equation.
