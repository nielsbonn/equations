...
Now we calculate
\begin{align*}
& \norm{\left(N f_N V_N \ast \lvert \ph\rvert^2 \right)\ph}_{L^2}^2 \\
...
& + \left( N f_N \ast \cc{\ph} \nabla \ph \right) \ph \label{eq:secondline}\\
& + \left( N f_N V_N \ast \cc{\nabla \ph} \ph \right) \ph. \label{eq:thirdline}
\end{align}
Clearly, $\norm{\nabla \Delta \ph}_{L^2} \leq \norm{\ph}_{H^3}$. Now for line \eqref{eq:firstline}, we have the estimate
\begin{align*}
\norm{\left( N f_N V_N \ast \lvert \ph \rvert^2 \right)\nabla \ph}_{L^2}^2 & = \int \di x \lvert \nabla \ph(x)\rvert^2 \lvert \int \di y N f_N V_N(y) \lvert\ph(x-y) \rvert^2\rvert^2\\
& \leq \int \di x \lvert \nabla \ph(x)\rvert^2 \norm{\ph}_\infty^4 \lvert \int \di y N f_N V_N(y) \rvert^2 \\
& = \norm{\nabla \ph}_{L^2}^2 \norm{\ph}_\infty^4 (8 \pi a_0)^2. 
\end{align*}
Similarly, we show that the $L^2$-norms of \eqref{eq:secondline} and \eqref{eq:thirdline} are bounded by $C \norm{\ph}_{H^2}^2 \norm{\ph}_{H^3}^2$. We then conclude that
\bd
\norm{\nabla \phdot}_{L^2} \leq \norm{\ph}_{H^3} + C \norm{\nabla \ph}_{L^2} \norm{\ph}_\infty^2 + C \norm{\ph}_{H^2} \norm{\ph}_{H^3} \leq C \norm{\ph}_{H^3}^2.
\ed
The bound for $\norm{\Delta \phdot}_{L^2}$ is proven by the same strategy.

(iii) \marginpar{to be typed}
TODO
\end{proof}

\begin{lem} \label{lm:kbounds} Let $k(x,y) = -N w_N(x-y) \ph(x) \ph(y)$.
\begin{enumerate}
