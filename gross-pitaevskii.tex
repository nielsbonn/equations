...
The time derivatives of the terms in lines \eqref{l10} through \eqref{l15} are bounded by $\Ncal^2/N$, $\Ncal$ and $\tilV$ (and as a consequence, by $\Ncal$ and $\Lcal$). (I'll type these estimates when I'm back. The method of proof is the same as in chapter \ref{ch:generatorestimates}, but easier because $a^\ast(\dot p_x) = \partial_t a^\ast(c_x)$ is regular while $a^\ast(c_x)$ is singular.)
