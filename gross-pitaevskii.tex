  ...
  \begin{enumerate}
    ...
    \item \label{lw2} Let $R$ be such that $\text{supp }V \subset \{ x \in
      ...
      \[
        w_N(x) = \frac{a_0}{N|x|}. 
      \]
    \item \label{lw3} It follows from \ref{lw1} and \ref{lw2} that, for all $x
      \in \R^3$,
      \[
    ...
    \item \label{lw4} There exist constants $C_1$ and $C_2$, depending only on
      ...
      \[
        ...
        |x|^2}.
      \]
    \item \label{lw5} It follows from \ref{lw4} that, for all $x \in \R^3$,
      \[
  ...
  \begin{enumerate}
    ...
    \item \label{kr} For almost all $x,y \in \R^3$,
      \[
        ...
        \varphi \|_{H^1}} |\varphi(x)| |\varphi(y)|.
      \] 
    \item \label{sup} Let $p$ and $s$ be the integral kernels in Lemma
      \ref{l:bt}. If $\varphi \in H^2(\R^3)$, then
      \[
...
\begin{proof}[Proof of Lemma \ref{l:bt}]
  ...
  We now prove the estimates for the integral kernels $p$, $r$ and $s$. Let
  $fg$ be a product of integral kernels. Then, by H\"older's inequality, $\| f
  g \|_{L^2} \le \| f \|_{L^2} \| g \|_{L^2}$. Hence, applying this to the
  series for $p$, we obtain
  \[
...
\begin{proof}[Proof of Lemma \ref{l:kernels}]
  ...
  \end{align*}
  This proves the first two estimates of part \ref{k}. Now,
  \[
  ...
  \ref{kr} From Lemma \ref{l:w}\ref{lw1}, it follows that $w_N < 1$. Hence,
  \[
    |k(x,y)| = N w_N(x-y) |\varphi(x)| |\varphi(y)| < N |\varphi(x)|
    |\varphi(y)|.
  \]
  This proves the first estimate. To bound $|r(x,y)|$, first observe that, by
  H\"older's inequality, Proposition \ref{p:wphi}, and part \ref{k},
  \begin{align*}
...
\begin{lem} \label{l:ap}
  ...
  \begin{enumerate}
    ...
    \item \label{l:ap2} There exists a constant $C_2$, uniform in $N$, that
      ...
      \[
        \begin{split}
          U_t^* \K \N U_t & \le C_2 \big( N U_t^* \K U_t + N U_t^* (\N+1) U_t
          +  (\N^2 + N(\N+1)) \\
          & \quad + T^* \phi(\varphi) W^*( \mathcal{H}_N + \N ) W
        \phi(\varphi) T + N^{-1} T^* \N W^*( \mathcal{H}_N + \N ) W \N T \big).
        \end{split}
      \]
  \end{enumerate}
\end{lem}
...
\begin{prp} \label{p:f}
  ...
  \end{align}
  where $C$ and $S$ are integral operators on $L^2(\R^3)$, with integral
  kernels $c$ and $s$, respectively, as in Lemma \ref{l:bt}\ref{l:bt2}.
\end{prp}
...
\begin{proof}[Proof of Proposition \ref{p:f}]
  ...
  follow easily by a short calculation using parts \ref{l:W1} and \ref{l:W3}
...
\begin{proof}[Proof of Proposition \ref{p:TNT}]
  ...
  \begin{equation} \label{c1}
    1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2 \le e^{C_1 (1+\| k \|_{L^2})}
  \end{equation}
  and $\| \nabla_1 s \|_{L^2}^2 \le e^{C_1 (1+\|k \|_{L^2})} \| \nabla_1 k
  \|_{L^2}^2$. All this together proves parts \eqref{TNT} and \eqref{TKT}.
...
\begin{proof}[Proof of Lemma \ref{l:ap}]
  ...
  Observe that, by Propositions \ref{l:kernels}\ref{k} and \ref{p:energy},
  there exists a constant $C$, uniform in $N$ and $t$, that depends only on
  $\| \varphi \|_{H^1}$ and $V$, such that
  ...
  \begin{align*}
    ...
    \psi \|^2 + 4\| \N T \psi \|^2.
  \end{align*}
  Thus, using again Proposition \ref{p:TNT}, and recalling \eqref{c1},
  \begin{align*}
    & \langle T_t U_t \psi, \N^2 T_t U_t \psi \rangle \\
  ...
  \end{align*}
  for some constants $C_1$ and $C_2$ that depend only on $\| k_t \|_{L^2}$.
  ...
  \begin{align*}
    ...
    & \le C_3 \big( N \langle U_t \psi, (\N+1) U_t \psi \rangle + N \langle
    \psi, (\N+1) \psi \rangle + \langle \psi, (\N+1)^2 \psi \rangle \big),
  \end{align*}
  for some constant $C_3$ that depends only on $\| k_t \|_{L^2}$. In view of
  \eqref{c2}, there is a constant $C_4$, uniform in $N$ and $t$, that depends
  only on $\| \varphi \|_{H^1}$ and $V$, such that $C_3 < C_4$. This concludes
  the proof of part \ref{l:ap1}.
  ...
  choosing $\varepsilon$ so that $\varepsilon C < 1/2$, applying Lemma
  \ref{l:kernels} to bound $\| \nabla_1 p \|_{L^2}$ and $\| \nabla_1 s
  \|_{L^2}$, and recalling \eqref{c2}, we obtain
  \begin{equation}
    \label{nk3}
    \begin{split}
      \langle U_t \psi, \K \N U_t \psi \rangle & \le C N \langle U_t \psi,
      \K U_t \psi \rangle + C (1 + \| \nabla_1 s \|_{L^2}^2) \langle U_t
      \psi, (\N+1) U_t \psi \rangle \\
      & \qquad + \frac{C}{N} \| (T_t \K T_t^*)^{1/2} T_t U_t T^* ( \N +
      \sqrt{N} \phi(\varphi) ) T \psi \|^2,
    \end{split}
  \end{equation}
  where $C$ is a constant, uniform in $N$ and $t$, that depends only on $\|
  ...
  \begin{align*}
    ...
    \psi \|^2 + \| \nabla_1 k \|_{L^2}^2 ( \| \N T \psi \|^2 + N \|
    \phi(\varphi) T \psi \|^2) \big) \\
    & \le C \big( \| \K^{1/2} T_t U_t T^* ( \N + \sqrt{N} \phi(\varphi) ) T
    \psi \|^2 + N \langle \psi, (\N^2 + N(\N+1)) \psi \rangle \big),
  \end{align*}
  for some constant $C$ as above. Now, since $V \ge 0$, note that $\K \le
  ...
  \begin{align*}
    ...
    & \le C \| \varphi_t \|_{H^2}^2 \big( N \langle \psi, (\N^2 + N (\N + 1))
    \psi \rangle + N \langle W \phi(\varphi) T \psi, (\mathcal{H}_N + \N)
    W \phi(\varphi) T \psi \rangle \\
    & \quad + \langle W \N T \psi, (\mathcal{H}_N + \N) W \N T \psi \rangle
    \big),
  \end{align*}
  for some constant $C$ as above. Therefore, combining the last two
  inequalities with \eqref{nk3},
  \begin{align*}
    \langle U_t \psi, \K \N U_t \psi \rangle & \le C \big( N \langle U_t
    \psi, \K U_t \psi \rangle + N \langle U_t
    \psi, (\N+1) U_t \psi \rangle \\
    & \quad + \langle \psi, (\N^2 + N (\N + 1)) \psi \rangle + \langle W
    \phi(\varphi) T \psi, (\mathcal{H}_N + \N) W \phi(\varphi) T \psi \rangle
    \\
