...
INSERTED \marginpar{please integrate in a way you like}: ------------------------------------------------------\\
We also make use of the following estimates (with $C$ independent of $N$ and $t$): \marginpar{I have a proof for this, but we should discuss first who edits the proof in the file.}
\bd
...
\marginpar{It's clear, but perhaps we should mention that these functions
converge as $N \to \infty$. Is $b(N,t) \to 0$ as $N \to \infty$?}
\bd
...
&+ \hc
\end{align}
The cancellations enable us to give the necessary estimates for all terms, see chapter \ref{ch:generatorestimates}.

\section{Estimates for the terms of the generator}
\label{ch:generatorestimates}
In this chapter we denote the expectation value $\scal{\psi}{A\psi}$ by $\ev{A}$. In this chapter, $C$ is a constant which can change from line to line in inequalities, but which is always independent of $N$ and $t$.
...
\end{split}
\end{equation}
\estlist{9}{2} For this term we need two estimates, too. The first one is just that the term is non-negative:
\begin{equation}
\tag{A}
\label{eq:bound2A}
\begin{split}
& \scal{\psi}{\frac{1}{2}\int \di x\, a^\ast(-\Delta_x s_x)a(s_x)\psi} \\
= & \frac{1}{2}\int \di x\, \norm{a(\nabla_x s_x)\psi}^2 \geq 0. 
\end{split}
\end{equation}

For the second estimate, notice that, using the product rule, integration by parts and the fact that $-\nabla_x w_N(x-y) = \nabla_y w_N(x-y)$, we have
\begin{align*}
...
\leq & \frac{1}{2} \sum_{i=1}^2 \lvert \scal{\psi}{\int \di x\di y\, b_i(x,y) a^\ast_x a^\ast_y \psi }\rvert \\
\leq & \frac{1}{2} \sum_{i=1}^2 \int \di y \norm{a_y \psi} \norm{a^\ast(b_i(\cdot,y))\psi} \\
\leq & \frac{1}{2} \sum_{i=1}^2 \left( \int \di y_1\, \norm{a_{y_1}\psi}^2 \int \di y_2\, \norm{b_i(\cdot,y_2)}^2 \norm{(\Ncal+1)^{1/2} \psi}^2 \right)^{1/2} \\
\leq & \ev{\Ncal+1} \frac{\norm{f_1}+\norm{f_2}}{2}.
 \end{align*}
Proof of the second estimate:
 \begin{align*}
& \lvert \scal{\psi}{\frac{1}{2} \int \di x\di y \left( f_1(x,y) a^\ast_x a_y + f_2(x,y) a_x a^\ast_y \right)  \psi} \rvert\\
\leq & \frac{1}{2} \lvert \scal{\psi}{\int \di x\di y\, f_1(x,y) \psi} \rvert + \frac{1}{2}\lvert \scal{\psi}{\int \di x \di y\, f_2(x,y) \psi}\rvert + \frac{1}{2} \lvert \scal{\psi}{\int \di x \di y\, f_2(x,y) \delta(x-y) \psi}\rvert \\
\leq & \frac{1}{2} \int \di x\, \norm{a_x \psi} \norm{a(f_1(x,\cdot))\psi} + \frac{1}{2} \int \di y\, \norm{a_y\psi} \norm{a(f_2(\cdot,y))\psi} + \frac{1}{2} \lvert \int \di x f_2(x,x) \rvert \scal{\psi}{\psi} \\
\leq & \frac{1}{2} \left( \int \di x_1 \norm{a_{x_1}\psi}^2 \int \di x_2 \norm{f_1(x_2,\cdot)}^2 \norm{\Ncal^{1/2}\psi}^2 \right)^{1/2}\\  & \quad + \frac{1}{2} \left( \int \di x_1 \norm{a_{x_1}\psi}^2 \int \di x_2 \norm{f_2(\cdot,x_2)}^2 \norm{\Ncal^{1/2}\psi}^2 \right)^{1/2} + \frac{1}{2}\lvert \int \di x\, f_2(x,x) \rvert \scal{\psi}{\psi} \\
\leq & \frac{1}{2}\ev{\Ncal} \left(\left(\int \di x \norm{f_1(x,\cdot)}^2 \right) + \left(\int \di x \norm{f_2(\cdot,x)}^2 \right) \right) + \frac{1}{2}\lvert \int \di x\, f_2(x,x) \rvert \scal{\psi}{\psi}. 	\qedhere
 \end{align*}
\end{proof}

\begin{lem}[Regularity of $\phdot$]
...
\begin{align*}
\norm{\dot k}^2 & = \int \di x\di y\, \lvert \frac{\di}{\di t} \left( -N w_N(x-y) \ph(x)\ph(y) \right) \rvert^2 \\
...
\end{align*}
Now use the estimate from the first part of the lemma.
\end{proof}
...
\end{itemize}
\end{lem}
\begin{proof} The proof is by induction in $n$. In the inductive step, we have to treat the cases of even $n$ and odd $n$ separately (though the calculations are similar).\\
\underline{basis:}\\
...
\underline{inductive step, case of even $n$:}\\
...
\end{align*}
so, as $n+1$ is odd, we have the correct expression for $\ad^{n+1}_B(\dot B)$. We still have to check validity of the estimates.
Clearly
\be{normnorm}
\begin{split}
...
\end{split}
\ee
where by abuse of notation, $x$ and $z$ are actually the variables which are integrated over in calculating the $L^2(\Rbb^3\times \Rbb^3)$-norm (see next equation).
...
\ed
which was to be proven. The calculation for $\norm{f_{n+1,2}}_{L^2}$ works the same way. We still have to proof the second extimate of the lemma, so we calculate using Hoelder's inequality that
\bd
\int \lvert f_{n+1,1}(x,x) \rvert \di x \leq 2\norm{k} \norm{f_{n,2}} \leq 2\norm{\dot k} (2\norm{k})^2
\ed
and in the same way $\int \lvert f_{n+1,2}(x,x) \rvert \di x$. Herewith the inductive step startin from even $n$ is proven.\\
\underline{inductive step, case of odd $n$:}\\
We calculate (by using the CCR and appropriately renaming integration variables)
\begin{align*}
\ad^{n+1}_B(\dot B) & = \left[ \frac{1}{2}\int \di x \di y \left( k(x,y)a^\ast_x a^\ast_y - \cc{k(x,y)} a_x a_y \right) , \frac{1}{2}\int \di x\di y \left( f_{n,1}(x,y) a^\ast_x a_y + f_{n,2}(x,y) a_x a^\ast_y \right) \right] \\
...
\end{align*}
so, as $n+1$ is even, we have the correct expression for $\ad^{n+1}_B(\dot B)$. Validity of the estimate for the $\norm{f_{n+1},1}$ and $\norm{f_{n+1},2}$ again follows by invoking Hoelder's inequality and the inductive hypothesis.\newline
...
There exists a constant $C$ independent of $N$ and $t$ such that the following estimate holds for all $t \in \Rbb$ and all $\psi \in \fock$:
 \bd
  \lvert \scal{\psi}{(\partial_t T^*_t)T_t \psi} \rvert \leq 2\norm{\dot k} \ev{\Ncal+1} e^{2\norm{k}} \leq C \norm{\ph}_{H^2}^2 \ev{\Ncal+1}.
 \ed
\end{lem}
\begin{proof}
This theorem is proved by expanding the time derivative as a series,
\bd
...
For any arbitrary $\psi \in \fock$, the expectation value $\ev{\cdot} = \scal{\psi}{\cdot\psi}$ satisfies the estimate
 \bd
...
\begin{lem}
  ...
  \begin{align}
    U_t^* \N^2 U_t & \apprle C^4 \big( N U_t^* (\N+1) U_t + N (\N+1) +
    (\N+1)^2 \big), \tag{i} \\
    U_t^* \K \N U_t & \apprle \tag{ii}
  \end{align}
...
\begin{prp}
  \label{p:TNT}
  Let $p, s \in L^2(\R^3 \times \R^3)$ and $\varphi \in H^1(\R^3)$. Then,
  \begin{align}
    ...
    \N \psi \rangle| + C^2 \langle \psi, \K \psi \rangle + C \| \nabla_1 s
    \|_{L^2}^2, \label{TKNT} \tag{iv} \\
    \phi(\varphi) \K \phi(\varphi) & \apprle \| \varphi \|_{L^2}^2 \K (\N+1) +
    \| \nabla \varphi \|_{L^2}^2 (\N + 1), \label{fkf} \tag{v}
  \end{align}
  where $C = 1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2$. Moreover, the same
...
\begin{proof}[Proof of Proposition \ref{p:formulae}]
  ...
  We only give an outline of the proof. Recall that $\phi(\varphi) =
...
\begin{proof}[Proof of Proposition \ref{p:TNT}]
  ...
  \end{align*}
  \marginpar{Applying (i) pointwise in $x$}
  \marginpar{Should we say a few words about this pull-through formula with
  sqrt?}
  ...
  \end{align*}
  Then, by part (i),
  \marginpar{Again part (i) pointwise in $x$}
  \begin{align*}
  ...
  \end{align*}
  Therefore, combining this with \eqref{ep5}, and using part (iii), we obtain
  \begin{align*}
    \langle \psi, T^* \K \N T \psi \rangle & \apprle C |\langle \psi, T^* \K T
...
\begin{proof}[Proof of Lemma \ref{l:N2}]
  (i) Let $C = 1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2$. By Proposition
  \ref{p:TNT},
  \begin{equation}
    \label{ep4}
    \begin{split}
  ...
  \begin{align*}
    ...
    \|^2 + 4\| \N T \psi \|^2,
  \end{align*}
  where we have used that $\| \varphi_t \|_{L^2} = \| \varphi \|_{L^2} = 1$.
  ...
  \end{align*}
  where $C = 1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2$. Therefore, recalling
  \eqref{ep4},
  \begin{align*}
  ...
  (ii) Applying parts (iv) and (iii) of Proposition \ref{p:TNT}, we find that
  \begin{align*}
    \langle \psi, U_t^* \K \N U_t \psi \rangle & = \langle T_t U_t \psi, T_t
  ...
  recalling that $\N$ commutes with $\mathcal{H}_N$, we get
  \begin{align*}
    ...
    & \le N \langle U_t \psi, \K U_t \psi \rangle + \sqrt{N} | \langle T_t U_t
    \psi, T_t \K T_t^* ( \phi(\varphi_t) + 2 \sqrt{N} ) T_t U_t \psi \rangle |
    \\
    & \quad + | \langle T_t U_t \psi, T_t \K T_t^* W_t^* e^{-it \mathcal{H}_N}
    W (\N + \sqrt{N} \phi(\varphi) + N) T \psi \rangle | \\
    & \le 4 N \langle U_t \psi, \K U_t \psi \rangle + \sqrt{N} | \langle T_t
    U_t \psi, T_t \K T_t^* \phi(\varphi_t) T_t U_t \psi \rangle | \\
    & \quad + | \langle T_t U_t \psi, T_t \K T_t^* T_t U_t T^* (\N + \sqrt{N}
    \phi(\varphi) ) T \psi \rangle|.
  \end{align*}
  We next estimate the last two terms in this inequality.


  First, by Cauchy-Schwarz inequality and Proposition \ref{p:TNT}(v),
  \begin{align*}
    ...
    & \le \frac{N}{\varepsilon} \langle U_t \psi, \K U_t \psi \rangle +
    \varepsilon \langle U_t \psi, T_t^* \phi(\varphi_t) T_t \K T_t^*
    \phi(\varphi_t) T_t U_t \psi \rangle \\
    & = \frac{N}{\varepsilon} \langle U_t \psi, \K U_t \psi \rangle +
...
\marginpar{Strichartz version can be removed?}
\iffalse
\subsection{$\varphi_t^{(N)}$ converges to $\varphi_t$ in $L^2$ (using
