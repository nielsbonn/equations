...
\begin{lem}
  ...
  \begin{align}
    ...
    \big), \tag{i} \\
    U_t^* \K \N U_t & \le C_2 \tag{ii}
  \end{align}
  where $C_1$ is a constant that depends only on $\| p \|_{L^2}$ and $\| s
  \|_{L^2}$, and $C_2$ is a constant that depends only on $\| p \|_{L^2}$, $\|
  s \|_{L^2}$, $\| \varphi_t \|_{H^2}$ and $\| \nabla_1 p \|_{L^2}$.
\end{lem}
...
\begin{prp}
  \label{p:formulae}
  Let $\varphi \in L^2(\R^3)$. The following pull-through formulae hold true:
  \begin{align}
    \N W(\sqrt{N} \varphi)^* & = W(\sqrt{N} \varphi)^* (\N - \sqrt{N}
    \phi(\varphi) + N \| \varphi \|_{L^2}^2), \tag{i} \\
    \N W(\sqrt{N} \varphi) & = W(\sqrt{N} \varphi) (\N + \sqrt{N}
    \phi(\varphi) + N\| \varphi \|_{L^2}^2), \tag{ii} \\
...
\begin{proof}[Proof of Lemma \ref{l:N2}]
  ...
  (ii) Applying parts (iv) and (iii) of Proposition \ref{p:TNT}, we get
  \begin{equation}
    \label{nk1}
    \begin{split}
  ...
  \end{align*}
  Thus, combining this with \eqref{nk2} and \eqref{nk1}, and choosing
  ...
  \end{align*}
  Therefore, combining the last two inequalities with \eqref{nk3}, we find
  that
  \begin{align*}
