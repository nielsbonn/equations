...
\author{Niels Benedikter, Gustavo de Oliveira and Benjamin Schlein\\ \\Institute of Applied Mathematics, University of Bonn\\ Endenicher Allee 60, 53115 Bonn, Germany}
\title{Rate of Convergence towards the Gross-Pitaevskii limit of interacting Bose-Einstein condensate (DRAFT)}
...
We denote the projection onto the state $\varphi_t \in L^2(\Rbb^3)$ using the Dirac bra-ket notation by $\project{\varphi_t}$.

We can now formulate the main theorem which provides estimates for the difference of the expectation values of one-particle observables w.\,r.\,t.\ the Schr\"odinger evolution and the effective evolution. Notice that while the Schr\"odinger equation is linear, the Gross-Pitaevskii equation is a non-linear PDE. The advantage of the Gross-Pitaevskii equation is that it is a PDE in $L^2(\Rbb^3)$ while the Schr\"odinger equation is formulated in $L^2(\Rbb^{3N})$ with typically very large $N$.

\begin{thm}[Evolution of Factorized States] \label{thm:main_factorized}\marginpar{The Theorem for factorized initial data is not proven correctly, see remark at Lemma 5.1. Only for Squeezed Coherent Initial Data, we have a Theorem (see Thm. 5.3).}
...
\begin{lem} \marginpar{This Lemma is incorrect: We can not use initial data without $T_0$, because we dropped a constant of order $N$ in defining the fluctuations. (The $t=0$ contrib in Gronwall will thus be large of order $N$.)}
\label{lem:TrJ}
