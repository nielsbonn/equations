...
that are symmetric with respect to arbitrary permutations of the $n$ variables
in $\R^3$. Vectors in $\mathcal{F}$ are sequences $\psi = \{ \psi^{(n)} \}_{n
...
We will also make use of operator valued distributions $a_x^*$ and $a_x$, with
$x \in \R^3$, defined so that
\begin{equation}
  \label{a}
  \begin{aligned}
...
\begin{lem}[\cite{ESY2010}]
  ...
  \[
    ...
    \frac{C \rho}{|x|}, \qquad |\nabla^2 w_N(x)| \le \frac{C \rho}{N^2 |x|^2}
  \]
  for all $x \in \R^3$.
\end{lem}


\section{Bogoliubov transformation (duplicated; see next section)}
