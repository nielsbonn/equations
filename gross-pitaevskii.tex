...
\bd
\ev{K} + \ev{\tilV} \leq 2 \ev{\Lcal_N(t)} + C \norm{\ph}_{H^2}^4 \ev{\frac{1}{N}\Ncal^2 + \Ncal + 1}.
\ed
As $\Kcal$ and $\tilV$ are non-negative operators, the same upper bound holds for each of $\Kcal$ and $\tilV$ individually.
\end{lem}
\begin{proof}
We know that $\Lcal_N(t) = (i\partial_t T^\ast_t)T_t + T^\ast_t \Lcal_N^{(0)}(t)T_t$ and we have explictly calculated and bounded $T^\ast_t \Lcal_N^{(0)}(t)T_t$. We now only write line \eqref{l9} and the first summand in line \eqref{l10} explicitly, all the other terms we collect in $\Lcal_{\textrm{rest}}$:
\begin{align*}
