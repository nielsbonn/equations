...
\rfoot{Page \thepage}
%\rfoot{{\footnotesize \LaTeX: \today, \thistime
%\\ git commit: \gitAuthorIsoDate }}
...
\begin{itemize}
 ...
 \item pull out $\sup_x \norm{s_x}^2$, $\sup_x \norm{p_x}^2$ and $\norm{\ph}_\infty$ from the integrals integrals, then $\int \di y\, NV_N(x-y) = b_0$ (independent of $x$) can be used
\item use Cauchy-Schwarz in such a way that at most two operators $a^\#$ act on each $\psi$. Usually Hoelder (with $p=q=2$) is applied to all integrals, so that we get an upper bound by a sum of two terms. In using Hoelder, we always split $V_N$ into two square roots, so that we get $V_N$ in each integral, never $V_N^2$. ($\int N V_N$ scales like $N^0$, but for other powers of $N V_N$ integrals do depend on $N$ non-trivially.)
...
\ed
(Notive that through $c$, the operator $\tilV$ is time-dependent. However, as this does not play any role in our proof, we do not indicate this time-dependence and $\tilV$ should always be read as belonging to the same time $t$ like the other objects in the equation where it appears.)
...
We still have to give a bound for the $(\partial_t T^*_t)T_t$-part of the generator. We start with a number of simple lemmata.
\begin{lem}
\label{lm:Bbound}
...
\begin{lem}
\label{lem:ldotbounds} There exists a constant $C$ independent of $N$ and $t$ such that for all $\psi$
 \bd
  \lvert \scal{\psi}{\dot \Lcal_N(t) \psi} \rvert \leq C \norm{\ph}_{H^4}^8 \scal{\psi}{\left(\frac{1}{N}\Ncal^2 + \Ncal + 1 + \Kcal + \tilV \right)\psi}.
 \ed
\end{lem}
\begin{proof}
Combining Lemma \ref{lm:timederivative} and Lemma \ref{lm:kbounds}, we obtain the estimate
\bd
\lvert\scal{\psi}{\partial_t\left((\partial_t T^\ast_t) T_t \right)\psi}\rvert \leq C \norm{\ph}_{H^4}^4 \ev{\Ncal+1}.
\ed
(We make no effort to optimize the appearance of $H^n$ norms, just take $n$ and the power as high as you like.)

The term $\scal{\psi}{\partial_t( T^\ast_t \Lcal_N^{(0)}(t) T_t )\psi}$ can be estimated by the same methods which where already used for estimating $\scal{\psi}{ T^\ast_t \Lcal_N^{(0)}(t) T_t \psi}$, however, additionally the following objects are used:
...
There exists a constant $C$ independent of $N$ and $t$ (but possibly depending on $\norm{\ph}_{H^1}$) such that for all $\psi \in \fock$, the expectation value $\ev{\cdot} = \scal{\psi}{\cdot\psi}$ satisfies
 \bd
  \lvert \ev{[\Lcal_N(t),\Ncal]} \rvert \leq C \norm{\ph}_{H^2}^2 \left( \ev{\Ncal+1} + \frac{1}{N}\ev{\Ncal^2} + \ev{\tilV} + \ev{\Kcal} \right).
 \ed
\end{lem}
\begin{proof}
...
Now use the estimates from chapter \ref{ch:generatorestimates} (choose e.\,g.\ $\varepsilon = 1$) to bound all summands of the commutator $[T^\ast_t \Lcal_N^{(0)}(t)T_t,\Ncal]$.
...
\begin{prp}
\label{prp:Nbound} \marginpar{to be doublechecked}
Let $\scal{\Psi}{\left(\frac{\Ncal^2}{N} + \Ncal \right)\Psi} \leq C$ and $\scal{T^\ast_0 \Psi}{\Lcal_N(0)T^\ast_0 \Psi} \leq C$, where $C$ is independent of $N$. Then
\bd
0 \leq \scal{U_t T^\ast_0 \Psi}{\left(\Ncal+1\right)U_t T^\ast_0 \Psi} \leq C e^{K_1 e^{K_2 t}},
\ed
where $K_1$, $C$, $K_2$ are independent of $N$ and $t$.
\end{prp}
\begin{proof}
...
\begin{rem}\marginpar{to be doublechecked}
 We have the following examples satisfying the assumptions of Proposition \ref{prp:Nbound}:
\begin{itemize}
 \item $\Psi = \Omega$. This corresponds to a factorized initial state.
 \item $\Psi = T_0 \tilde \Psi$ where $\tilde \Psi$ satisfies $\scal{\tilde \Psi}{\Hcal_N \tilde \Psi} \leq C$. (This can be seen employing Lemma \ref{lem:tilV}.) We expect that states showing BEC can be treated in this way.
\end{itemize}
\end{rem}

\begin{lem}\marginpar{to be doublechecked}
\label{lem:tilV}
 For all $t$, in every state $\zeta$ the expectation value of $\tilV = \tilV(t)$ can be estimated as follows:
\bd
\scal{\zeta}{\tilV \zeta} \leq C \scal{\zeta}{\Vcal_N \zeta} + C \norm{\ph}_{H^2}^2\scal{\zeta}{\frac{(\Ncal+1)^2}{N} \zeta}.
\ed
\end{lem}
\begin{proof}
Plug in $a(c_x) = a_x + a(p_x)$, multiply out and use Hoelder's inequality (sum version).
\end{proof}
...
\begin{lem}
\label{lem:phNtoph}
Let $J$ be a Hilbert-Schmidt operator. Then there exists a constant $C$ independent of $N$ and $t$ such that
\bd
