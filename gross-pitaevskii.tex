...
&- \hc
\end{align*}
Now choose e.\,g.\ $\varepsilon = 1$ and use the estimates from chapter \ref{ch:generatorestimates} to bound all summand of the commutator. Together with lemma \ref{lm:timederivative} for bounding $\ev{(\partial_t T^\ast_t) T_t}$, the theorem is now proven.
\end{proof}
...
\begin{prp}
  ...
  \begin{align}
    ...
    T^* \N \K T & \apprle \frac{C}{N} \N T^* \K T \N + C N T^* \K T + C T \K
    T^*, \label{TKNT} \tag{iv}
  \end{align}
  where $C = 1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2$. Moreover, the same
  inequalities hold with $T$ replaced by $T^*$.
\end{prp}
...
\begin{proof}[Proof of Proposition \ref{p:formulae}]
  ...
  (iii) follow easily by a short calculation using parts (ii) and (iii) of
  Lemma \ref{l:W}. Similarly, part (iv) follows from Lemma \ref{l:bt}.
\end{proof}
...
\begin{proof}[Proof of Proposition \ref{p:TNT}]
  ...
  \begin{align*}
    | \langle \psi, T^* \K T \N \psi \rangle| & = |\langle (T^* \K T)^{1/2}
    \psi, (T^* \K T)^{1/2} \N \psi \rangle| \\
    & \le N \langle \psi, T^* \K T \psi \rangle + \frac{1}{N} \langle \psi, \N
    T^* K T \N \psi \rangle.
  \end{align*}
  Therefore, combining the last two inequalities with \eqref{ep5}, we obtain
  \[
    \langle \psi, T^* \K \N T \psi \rangle \apprle \frac{C}{N} \langle \psi,
    \N T^* \K T \N \psi \rangle + C N \langle \psi, T^* \K T \psi \rangle +
    C \langle \psi, T \K T^* \psi \rangle,
  \]
  where $C = 1 + \| p \|_{L^2}^2 + \| s \|_{L^2}^2$. This completes the proof.
\end{proof}
...
\begin{proof}[Proof of Lemma \ref{l:N2}]
  ...
  \begin{equation}
    ...
    \begin{split}
      ...
      & \apprle C^2 \langle T_t U_t \psi, \N^2 T_t U_t \psi \rangle + C^2
      \langle T_t U_t \psi, (\N+1) T_t U_t \psi \rangle. \\
    \end{split}
    \end{equation} 
