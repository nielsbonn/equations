...
\begin{lem}
\label{lem:ldotbounds} There exists a constant $C$ independent of $N$ and $t$ such that
 \bd
  \lvert \scal{\psi}{\dot \Lcal_N(t) \psi} \rvert \leq C \norm{\ph}_{H^4}^8 \scal{\psi}{\left(\frac{1}{N}\Ncal^2 + \Ncal + 1 + \Kcal + \tilV \right)\psi}.
 \ed
\end{lem}
\begin{proof}
This is certainly the most terribble clumsy ugly proof ever. to be typed ASAP. Niels
\end{proof}
...
\ed
where $d_N = \norm{P_N W_0\Psi}^{-1}$ and $\gamma_{N,t}^{(1)}(x;y) = d_N^2 \scal{e^{-it \Hcal_N} P_N W_0 \Psi}{a^\ast_y a_x e^{-it \Hcal_N} P_N W_0 \Psi}$.
...
\begin{lem}
\label{lem:phNtoph}
 Convergence of $\Tr \lvert \project{\ph} - \project{\varphi_t} \rvert$ or something like that.
\end{lem}
\begin{proof}
elementary.
\end{proof}
