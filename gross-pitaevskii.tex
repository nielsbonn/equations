...
\begin{proof}
  ...
  Proposition \ref{p:reg1}, given $t > 0$, one can find constants $C$ and $T$,
  ...
  $T$ can be chosen uniformly in time) one easily finds that
  \[
...
\begin{proof}
  ...
  \end{align*}
  and let $R$ be such that $\text{supp }V \subset \{ x \in \R^3 \; | \;\; |x|
  \le R \}$. By Minkowski's, H\"older's, and Sobolev's inequalities,
  \begin{align*}
    ...
    - |\varphi_t|^2 \|_{L^3}.
  \end{align*}
  \marginpar{This could be omitted; or perhaps made shorter using the weak
  derivative\dots}
...
\begin{proof}
  ...
  and we are using standard multi-index notation and conventions. According to
  this we have $|\alpha| = \alpha_1 + \alpha_2 + \alpha_3$.
  ...
  \gamma_2|=k$ so that $|\gamma_1| \le k$ and $|\gamma_2| \le k$. Thus, using
  the above estimates to bound the factor with highest order derivative and
  the other two factors, we conclude that the term that we are considering is
  bounded by
  \begin{align*}
    ...
    \| \varphi_t \|_{H^{|\alpha|-k}}
  \end{align*}
  for $0 \le k \le |\alpha|-2$. Observe that by varying $k$ in this range, we
  can estimate all possible terms in \eqref{Dphi}. In fact, it follows that
  \begin{align*}
    \sup_{t \in [0,T]} \| \partial^\alpha \varphi_t \|_{L^2} & \le \|
    \partial^\alpha \varphi \|_{L^2} + C 4^{|\alpha|} \| V \|_{L^1} T^{1/2}
    \times \\
    & \quad \times \Big[ \sup_{t \in [0,T]} \| \varphi_t \|_{H^1}^2 \sup_{t
    \in [0,T]} \| \varphi_t \|_{H^{|\alpha|}} + \sup_{t \in [0,T]} \|
