...
Consider a function $f\colon \Rbb^3\times\Rbb^3 \to \Cbb$. To simplify the
notation we write
\be{no1}
  f_y(x) = f(x,y)
\ee
...
\begin{lem}[Bogoliubov transformation] \label{l:bt}
  ...
  \begin{alignat*}{2}
    ...
    s & = k + r, & \qquad r & = \sum_{n=1}^\infty \frac{1}{(2n+1)!}
...
\begin{prp} \label{p:wphi}
  Let $\varphi \in H^1(\R^3)$, and let $f_N=1-w_N$ be the solution of the
  zero-energy scattering equation. Then, for almost all $x,y \in \R^3$,
  \begin{align*}
...
% Funny behavior here. Forcing hyphenation...
  ...
  \begin{align*}
    \| \nabla_2 p \|_{L^2} & \le 2^{-1} e^{1+\| k \|_{L^2}} \| \nabla_2 k
    \overline{k} \|_{L^2}, \\
    \| \nabla_2 r \|_{L^2} & \le e^{1+\| k \|_{L^2}} \| \nabla_2
    \overline{k} k \|_{L^2}, \\
    \| \nabla_2 s \|_{L^2} & \le 2 e^{1+ \| k \|_{L^2}} \| \nabla_2 k
    \|_{L^2}.
  \end{align*}
...
& \apprle \int \di x\di y \lvert\varphi(x)\rvert^2 \lvert \varphi(y)\rvert^2
\left\lvert \int \di z \frac{\lvert\varphi(z)\rvert^2}{\lvert x-z\rvert
\lvert z-y\rvert^2} \right\rvert^2 \\
& \apprle \int \di y\di z_1 \di z_2 \lvert \varphi(y)\rvert^2 \frac{\lvert \varphi(z_1)\rvert^2}{\lvert y-z_1\rvert^2} \frac{\lvert\varphi(z_2)\rvert^2}{\lvert y-z_2\rvert^2} \int \di x \frac{\lvert \varphi(x)\rvert^2}{\lvert z_1-x\rvert \lvert z_2 - x\rvert} \\
...
\begin{proof}[Proof of Proposition \ref{p:TNT}]
  ...
  Proof of \eqref{TNT} and \eqref{TKT}: Write
  \[
  ...
  \begin{align*}
    ...
    \rangle + \| \nabla_2 s \|_{L^2}^2.
  \end{align*}
  Furthermore, by Lemma \ref{l:bt}, there exists a universal constant $C_1$
  such that
  ...
  Proof of \eqref{fKf}: By Cauchy-Schwarz inequality, a brief calculation,
  and Lemma \ref{l:a},
  \begin{align*}
