...
\author{HCM Bonn}
\title{Rate of Convergence towards Gross-Pitaevskii limit of interacting Bose-Einstein condensates}

\begin{document}
\maketitle

\begin{abstract}
The non-linear Gross-Pitaevskii equation describes the macroscopic dynamics of Bose-Einstein condensates in the limit of large number of particles $N$. In this paper we show that the microscopic quantum mechanical evolution converges towards the Gross-Pitaevskii dynamics if the two-particle interaction potential is scaled as $N^3 V(N(x_i-x_j))$. 
Our method is based on using the Bogoliubov transformation to implement a short-scale structure on coherent initial states, similar to squeezed coherent states known from quantum optics. The Gross-Pitaevskii limit can then be treated as a mean-field limit. We obtain explicit estimates on the rate of convergence of $k$-particle reduced density matrices.
\end{abstract}
...
\ed
We assume that the unscaled potential $V \in C^\infty_c(\Rbb^3)$ is
non-negative and spherically symmetric. \marginpar{*} We choose $R \in \Rbb$ such that $\operatorname{supp} V \subset \{x \in \Rbb^3 : \lvert x\rvert \leq R \}$. The scaling is chosen in such a way that for $N \to \infty$ formally $N^3 V(N(x_i-x_j)) \to \delta(x_i-x_j) b_0$, where $b_0 = \int V$.

\marginpar{*} We use the short-hand $V_N(x) = N^2 V(Nx)$.
...
\ed
\marginpar{*} which is clearly independent of $N$.
...
\marginpar{*} Furthermore, let $\varphi_t$ be the solution to the Gross-Pitaevskii equation with initial data $\varphi_0 = \varphi$.

 Then there exist constants $C$, $K_1$ and $K_2$, which do not depend on $N$
 and $t$, such that for any  \marginpar{*} Hilbert-Schmidt operator $J \in L^2(\Rbb^3 \times \Rbb^3)$ (a one-particle observable)
\bd
...
\begin{rem}
\begin{enumerate}
\marginpar{*} \item In the second-quantized setting we are working in it is more natural to consider the time evolution of squeezed coherent states instead of factorized states. For squeezed coherent states a better rate of convergence can be obtained, see Theorem \ref{thm:main_squeezed}.
\item \marginpar{*} As pointed out by Seiringer \cite{RS2009} we have $\Tr \big\lvert \gamma_{N}^{(1)} - \project{\varphi} \big\rvert \leq 2 \norm{\gamma_N^{(1)} - \project{\varphi}}_{HS}$ for any reduced density matrix $\gamma^{(1)}_N$. As the Hilbert-Schmidt operators form a Hilbert space with scalar product $\Tr A^\ast B$ we have $\norm{\gamma_N^{(1)} - \project{\varphi}}_{HS} \leq \sup_{\norm{J}_{HS}=1} \Tr \left( J \big( \gamma^{(1)}- \project{\varphi} \big) \right)$. Thus we have actually proven convergence in the trace norm
\bd
...
states. \marginpar{*} Rodnianski and Schlein \cite{RS2009} show that for solutions to the Schr\"odinger equation with Hamiltonian
\[
H_N = \sum_{j=1}^N (-\Delta_{x_j}) + \frac{1}{N}\sum_{i<j}^N U(x_i-x_j)
\]
and with factorized initial data the reduced density matrices convergence in trace-norm with a bound of the form $C e^{Kt}/\sqrt{N}$ to a solution of the Hartree equation
\[
i \partial_t \ph = -\Delta \ph + \left(U \ast \lvert \ph\rvert^2 \right) \ph.
\]
\marginpar{*} This bound was improved \cite{CLS2011} to a bound of the form
$C e^{Kt}/N$. Notice that formally for \marginpar{U a} $U = N V_N \to b_0
...
is $8 \pi a_0$, but $8 \pi a_0 \neq b_0$. \marginpar{*} (The result in that
paper is based on a compactness argument and does not give any information
about the rate of convergence.) \marginpar{*} This failure of naively taking
the limit has its origin in a singular short-scale structure of the $N$-body
quantum state $\psi_{N,t}$. \marginpar{*} The short-scale structure is given by the two-particle scattering equation \cite{EMS2009}.

\marginpar{*} Therefore we compare the evolution of coherent states with the effective evolution given by the modified Hartree equation
\be{modHartreeEqu}
i \partial_t \ph = -\Delta \ph + \left(N f_N V_N \ast \lvert \ph\rvert^2 \right) \ph,
\ee
\marginpar{f} which formally has the correct limiting equation.
Using the Bogoliubov transformation we can implement the short-scale structure in our effective dynamics. \marginpar{f} This formally corresponds to the use of squeezed coherent states known from quantum optics \cite{Loudon}. Mathematically, the Bogoliubov transformation explicitly exhibits cancellations in the generator of fluctuation dynamics around the effective evolution.

\marginpar{par} The argument is completed by showing that solutions of the modified Hartree equation converge to solutions of the Gross-Pitaevskii equation with coupling constant $8\pi a_0$.
\newline
...
\emph{Remarks on notation:} We use the symbol $C$ throughout the paper for constants which might be redefined from line to line, but which are always independent of $N$ and of time $t$. We sometimes comment explicitly on additional properties of constants $C$.
The notation
\[
  A \apprle B
\]
...
\marginpar{*} Notice also that though $k$ may depend on $t$ through $\ph$, we do not denote the $t$-dependence of $k$ explicitly, similarly for the other integral kernels $s$, $r$ and $p$. Some lemmata about the integral kernels are formulated for general $\varphi$ where the modified Hartree equation does not play any role.
...
\marginpar{*} We can extend the definition of one-particle reduced density matrices to states $\psi \in \fock$ by the following definition:
\be{fock_density}
\gamma_{\psi}^{(1)}(x;y) := \frac{1}{\scal{\psi}{\Ncal \psi}} \scal{\psi}{a^\ast_y a_x \psi}.
\ee
\marginpar{*} For $\psi$ in the $N$-particle subspace of $\fock$, $\psi \in L^2_s(\Rbb^{3N})$, this definition is equivalent to the definition in \eqr{red_density}.
...
\label{s:bogoliubov}
\marginpar{*} Consider a function $f\colon \Rbb^3\times\Rbb^3 \to \Cbb$ (an integral kernel). To simplify the
notation we write
\be{no1}
  f_y(x) = f(x,y)
\ee
...
\]
\marginpar{*} with normalization $\lim_{|x|\to\infty} f(x) = 1$. We will write
\[
  f = 1 - w
\]
...
\]
% where $R$ is such that $\text{supp} \ V \subset \{ x \in \R^3: |x| \le R \}$.
...
be dropped from now on without any further comment. \marginpar{*} (Notice that $\scal{U_N(t)T^\ast_0 \Psi}{(\Ncal+1)U_t T_0^\ast \Psi} = \scal{\tilde U_N(t)T^\ast_0 \Psi}{(\Ncal+1)\tilde U_t T_0^\ast \Psi}$ for all $\tilde U_N(t) = e^{i\theta_N(t)} U_N(t)$ with $\theta_N(t) \in \Rbb$.)
...
\marginpar{*} We first identify cancellations between linear terms originating from the transform \eqr{no4} of linear terms and linear terms originating from normal-ordering the transform of cubic terms
\begin{align*}
...
terms, thereby creating a \marginpar{*} commutator $\scal{c_x}{s_y}$. Now $\scal{c_x}{s_y} = \scal{\delta_x + p_x}{k_y + r_y} = k(x,y) + r(x,y) + \scal{p_x}{s_y}$, and the contribution of $k(x,y)$ cancels line \eqr{linearterm}. The result is
\begin{align*}
T^*_t \Lcal^{(0)}_N(t) T_t \qquad \\
= \int \di x\, T^*_t a^*_x T_t \bigg[ &   \frac{1}{2}(-\Delta_x T^*_t a_x T_t)  + \frac{1}{4}\int \di y V_N(x-y) T^*_t a^*_y a_y a_x T_t \\
& + \frac{1}{2} \int \di y\, NV_N(x-y)  \ph(x) \ph(y)  T^*_t a^*_y T_t \\
...
\eqr{no4}, then multiply out all the terms and normal-order them. To
simplify the expression a \marginpar{*} bit, notice that many terms appear twice, e.\,g.\ in the hermitian conjugate or with $x$ and $y$ interchanged. We obtain
\begin{align*}
& T^*_t \Lcal_N^{(0)}(t) T_t = \\ 
...
\ed
\marginpar{*}The term $\ev{\partial_t\big( T^\ast_t \Lcal_N^{(0)}(t) T_t \big)}$ can be estimated by the same methods which are used for estimating $\ev{ T^\ast_t \Lcal_N^{(0)}(t) T_t}$ in appendix \ref{ch:generatorestimates}, however, additionally the following quantities are used:
\begin{align*}
...
\end{align*}
These quantities are then estimated using the Lemmata \ref{lem:dottedests1}, \ref{lem:dottedests2} and \ref{lem:dottedests3}.

\marginpar{*}
(Remark: In $\partial_t\big( T^\ast_t \Lcal_N^{(0)}(t) T_t\big)$ there are about four times as many terms as in $T^\ast_t \Lcal_N^{(0)}(t) T_t$ because of the product rule, but many of them are easier to estimate because $\partial_t a^\ast(c_x) = a^\ast(\dot p_x)$ is regular while $a^\ast(c_x)$ is singular.)
\end{proof}
...
\marginpar{*} (choose e.\,g.\ $\varepsilon = 1$) to bound all summands of the commutator $[T^\ast_t \Lcal_N^{(0)}(t)T_t,\Ncal]$.

\marginpar{*}We expand $(\partial_t T^\ast_t)T_t$ as a series according to
\bd
...
\marginpar{*}So by assumption $u(0) \leq C$ for some constant $C$.
\end{proof}
...
\quad\text{and}\quad
  ...
  \end{align*}
  \marginpar{*}Hence, applying Cauchy-Schwarz inequality and Lemma \ref{l:a}, and observing
...
\ed
where $d_N = \norm{P_N W_0\Psi}^{-1}$ and $\gamma_{N,t}^{(1)} := \gamma_{\psi_{N,t}}^{(1)}$ is the one-particle reduced density matrix associated with the state $\psi_{N,t} = e^{-it \Hcal_N} d_N P_N W_0 \Psi$ (the operator $P_N$ is the projection on the $N$-particle subspace of Fock space).
%
%Remark: $\ph$ needn't be a solution of the Hartree equation here.
\end{lem}
\begin{proof}
We will use the definition \eqr{fock_density} of the reduced density matrix. 
...
& + \sqrt{N} \ph(y) \scal{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}{a^\ast_x W^\ast_t e^{-i \Hcal_N t} W_0 T_0 \Omega}\left(\norm{s}^2 + N\right)^{-1} \label{eq:squeez3}\\
& + \sqrt{N} \cc{\ph(x)} \scal{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}{a_y W^\ast_t e^{-i \Hcal_N t} W_0 T_0 \Omega} \left(\norm{s}^2 + N\right)^{-1} \label{eq:squeez2}\\
& + N \ph(y) \cc{\ph(x)} \scal{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}\left(\norm{s}^2 + N\right)^{-1}. \label{eq:squeez4}
\end{align}
...
\begin{fleqn}[1em]	% move formulas to left side in this section, make it look less chaotic.
...
\begin{thm} \label{t:pdes}% \label{c:reg2}
  ...
  Suppose that $f \in L^1(\R^3)$ and $V \in C_c^\infty(\R^3)$ with $fV \ge
