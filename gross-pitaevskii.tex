...
\begin{rem}
\begin{enumerate}
\item In the second-quantized setting we are working in it is more natural to consider the time evolution of squeezed coherent states instead of factorized states. For squeezed coherent states a better rate of convergence can be obtained, see Theorem \ref{thm:main_squeezed}.
\item As pointed out by Seiringer \cite{RS2009} we have $\Tr \big\lvert \gamma_{N}^{(1)} - \project{\varphi} \big\rvert \leq 2 \norm{\gamma_N^{(1)} - \project{\varphi}}_{HS}$ for any reduced density matrix $\gamma^{(1)}_N$. As the Hilbert-Schmidt operators form a Hilbert space with scalar product $\Tr A^\ast B$ we have $\norm{\gamma_N^{(1)} - \project{\varphi}}_{HS} \leq \sup_{\norm{J}_{HS}=1} \Tr \left( J \big( \gamma^{(1)}- \project{\varphi} \big) \right)$. Thus we have actually proven convergence in the trace norm
\bd
...
The method of our proof is based on the study of the evolution of coherent states. Rodnianski and Schlein \cite{RS2009} show that for solutions to the Schr\"odinger equation with Hamiltonian
\[
...
\]
This bound was improved \cite{CLS2011} to a bound of the form $C e^{Kt}/N$. Notice that formally for $U = N V_N \to b_0 \delta$ we obtain a Gross-Pitaevskii equation with coupling constant $b_0$. However, from \cite{ESY2010} it is known that the correct coupling constant is $8 \pi a_0$, but $8 \pi a_0 \neq b_0$. (The result in that paper is based on a compactness argument and does not give any information about the rate of convergence.) This failure of naively taking the limit has its origin in a singular short-scale structure of the $N$-body quantum state $\psi_{N,t}$. The short-scale structure is given by the two-particle scattering equation \cite{EMS2009}.

Therefore we compare the evolution of coherent states with the effective evolution given by the modified Hartree equation
\be{modHartreeEqu}
i \partial_t \ph = -\Delta \ph + \left(N f_N V_N \ast \lvert \ph\rvert^2 \right) \ph,
\ee
which formally has the correct limiting equation.
Using the Bogoliubov transformation we can implement the short-scale structure in our effective dynamics. This formally corresponds to the use of squeezed coherent states known from quantum optics \cite{Loudon}. Mathematically, the Bogoliubov transformation explicitly exhibits cancellations in the generator of fluctuation dynamics around the effective evolution.

The argument is completed by showing that solutions of the modified Hartree equation converge to solutions of the Gross-Pitaevskii equation with coupling constant $8\pi a_0$.
\newline
...
We can extend the definition of one-particle reduced density matrices to states $\psi \in \fock$ by the following definition:
\be{fock_density}
\gamma_{\psi}^{(1)}(x;y) := \frac{1}{\scal{\psi}{\Ncal \psi}} \scal{\psi}{a^\ast_y a_x \psi}.
\ee
For $\psi$ in the $N$-particle subspace of $\fock$, $\psi \in L^2_s(\Rbb^{3N})$, this definition is equivalent to the definition in \eqr{red_density}.
...
\bd
\ev{\Lcal_N(t)} + 4\left( C \norm{\ph}_{H^2}^2 \right)^2 \ev{\frac{1}{N}\Ncal^2 + \Ncal + 1} \geq \ev{\frac{1}{4}\Kcal+\frac{3}{4}\tilV} \geq \frac{1}{4} \ev{\Kcal + \tilV},
\ed
...
\ed
The term $\ev{\partial_t\big( T^\ast_t \Lcal_N^{(0)}(t) T_t \big)}$ can be estimated by the same methods which are used for estimating $\ev{ T^\ast_t \Lcal_N^{(0)}(t) T_t}$ in appendix \ref{ch:generatorestimates}, however, additionally the following quantities are used:
\begin{align*}
...
Thus we can use the estimates from appendix \ref{ch:generatorestimates} (choose e.\,g.\ $\varepsilon = 1$) to bound all summands of the commutator $[T^\ast_t \Lcal_N^{(0)}(t)T_t,\Ncal]$.
...
\begin{thm} \label{t:pdes}% \label{c:reg2}
  ...
  0$. For $N \ge 1$, consider a solution $\ph \in H^1(\R^3)$ of the modified
  Hartree equation
  \[
...
\begin{proof}
Collect Lemmata... comment on $V = \delta$... TODO. \marginpar{TODO.} 
\end{proof}
...
\marginpar{I modified this paragraph. Compare with your older version.}
...
\marginpar{\dots any norm of $V$ other than\dots(?)} $N f_N V_N$ other than
the $L^1$-norm, which is given by $\norm{N f_N V_N}_{L^1} = \| fV \|_{L^1}$.
The Gross-Pitaevskii equation can be formally \marginpar{\dots can be
formally seen\dots(?)} seen as the limiting equation of the modified Hartree
...
\begin{prp} \label{p:reg1}
  ...
  a solution $\varphi_t \in H^1(\R^3)$ of the Hartree equation
  \[
    i \partial_t \varphi_t = - \Delta \varphi_t + (V * |\varphi_t|^2)
    \varphi_t
  \]
...
\begin{prp} \label{p:lregn}
  ...
  with $V \ge 0$. Consider a solution $\varphi_t \in H^1(\R^3)$ of the
  Hartree equation
  \[
    i \partial_t \varphi_t = - \Delta \varphi_t + (V * |\varphi_t|^2)
    \varphi_t
  ...
  > 0$, depending only on $\| \varphi \|_{H^1}$, $\| V \|_{L^1}$ and $n$,
  such that
  \[
    \sup_{t \in [0,T]} \| \varphi_t \|_{H^n} \le C \| \varphi \|_{H^n} +
    \sup_{t \in [0,T]} \| \varphi_t \|_{H^{n-1}}^3,
  \]
  where $C$ is a constant that depends only on $n$. Furthermore, the
  constant $T$ decreases monotonically with respect to $\| \varphi
...
\begin{proof}
  ...
  \varphi \|_{H^1}$. In view of Proposition \ref{p:lregn}, for any $t > 0$,
  there exists an integer $j$, depending only on $B_1$ and $n$, such that
  $(j-1)T < t \le jT$. Consequently,
  \begin{align*}
  ...
  \end{align*}
  where $C$ is a constant that depends only on $n$. Now observe that, since
  $\| \varphi_t \|_{H^1} \le B_1$ for all $t \ge 0$, the constant $T$ can be
  chosen uniformly in time. Thus, we can iterate the above expression $j$
  times to cover the interval $[0,jT]$ and obtain
  \begin{align*}
