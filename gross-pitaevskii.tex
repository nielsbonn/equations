...
\ee
It has to be noticed that $\norm{\gradone k} \sim N^{1/2}$, which is not desired. Therefore, we always have to keep $\norm{\gradone k \cc k}$, which is of order $N^0$ according to Lemma \ref{l:kernels} \ref{k}.

Estimate for $\gradone \dot p$ (the proof for $\gradone \dot r$ works similarly and will be omitted):
\bd
\gradone \dot p = \frac{\partial}{\partial_t} \left( \sum_{n=1}^\infty (k \cc k)^{n-1} \gradone(k \cc k) \frac{1}{(2n)!} \right).
\ed
After using the product rule for the time derivative and \eqr{hoelder} we have the estimate
\begin{align*}
\norm{\gradone \dot p} & \leq \sum_{n=1}^\infty \frac{1}{(2n)!} \sum_{j=0}^{n-2} \norm{k}^{2j} \norm{\partial_t(k \cc k)} \norm{k}^{2(n-2-j)} \norm{\gradone (k \cc k)} \\
