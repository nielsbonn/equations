...
\begin{rem}
\begin{enumerate}
\item We remark that in the second-quantized setting we are working in it is actually more natural to consider the time evolution of squeezed coherent states instead of projecting them onto factorized states and thus a better bound on the rate of convergence can be obtained, cf.\ Theorem \ref{thm:main_squeezed}.
...
In \eqr{no3} we made use of a cancellation in the term $\Ocal({N^{1/2}})$ (term which is linear in creation/annihilation operators) due to $\ph$ satisfying the modified Hartree equation \eqr{modHartreeEqu}.
%\bd
...
& + \hc
\end{align*}
We proceed to identify a cancellation among the boxed terms. We exhibit the cancellation by expanding $c = \delta + p$ and $s = k + r$ and applying the product rule to $-\Delta_x k(y,x)$. Thus the lhs of the zero-energy scattering equation \eqr{scattering} appears. However, there are some (more regular) remainder terms left, see \eqref{l7}, \eqref{l8}, \eqref{l14} and \eqref{l18} in the final result for the generator below.
...
\begin{thm}[Time Evolution of Squeezed Coherent States] \label{thm:main_squeezed}
 Let $\psi_{N} = W(\sqrt{N} \varphi) T_0 \Omega$ for some $\varphi \in H^4(\Rbb^3)$ with $\norm{\varphi}_{L^2} = 1$. Denote by $\psi_{N,t} = e^{-it H_N}\psi_N$ the solution to the Schroedinger equation with initial data $\psi_{N,0}= \psi_N$. Let $\Gamma_{N,t} := \Gamma_{\psi_{N,t}}$ be the corresponding one-particle density matrix.
...
\begin{align}
& \scal{\psi_{N,t}}{a^\ast_y a_x \psi_{N,t}} \nonumber \\
& = \scal{W^\ast_t e^{-i \Hcal_N t} W_0 T_0 \Omega}{a^\ast_y a_x W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega} \label{eq:squeez1} \\
& + \sqrt{N} \cc{\ph(y)} \scal{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}{a_x W^\ast_t e^{-i \Hcal_N t} W_0 T_0 \Omega} \label{eq:squeez2}\\
& + \sqrt{N} \ph(x) \scal{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}{a^\ast_y W^\ast_t e^{-i \Hcal_N t} W_0 T_0 \Omega} \label{eq:squeez3}\\
& + N \ph(x) \cc{\ph(y)} \scal{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}{W^\ast_t e^{-i\Hcal_N t} W_0 T_0 \Omega}. \label{eq:squeez4}
\end{align}
Clearly line \eqref{eq:squeez4} is equal to $N \ph(x) \cc{\ph(y)}$. Using H\"older's inequality
\[
\left\lvert \frac{N}{\scal{\psi_{N,t}}{\Ncal \psi_{N,t}}} \int \di x \di y\, J(x,y) \ph(x) \cc{\ph(y)} - \Tr\left( J \project{\ph}  \right) \right\rvert \leq \frac{C}{N} \norm{J}_{HS},
\]
...
\intertext{and using Proposition \ref{p:TNT} (\ref{TNT}) we arrive at}
...
\intertext{and using Proposition \ref{p:TNT} (\ref{TN2T}) we get}
& \leq \frac{C}{\sqrt{N}} \norm{J}_{HS} \left( \frac{\scal{U_t \Omega}{(\Ncal+1)^2 U_t \Omega}}{N} \right)^{1/2}
\intertext{and using $N^{-1} \scal{\Omega}{U^\ast_t \Ncal^2 U_t \Omega} \leq C \scal{\Omega}{U^\ast_t \left(\Ncal + 1\right)U_t \Omega}$ (Corollary \ref{cor:N2}) we arrive at}
& \leq \frac{C}{\sqrt{N}} \norm{J}_{HS} \left( \frac{\scal{U_t \Omega}{(\Ncal+1) U_t \Omega}}{N} \right)^{1/2}
\end{align*}
...
\marginpar{I think this should not be called a corollary.}
\begin{cor} \label{l:pdes}% \label{c:reg2}
