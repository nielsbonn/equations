...
\marginpar{Corollary $\to$ consequence (?)}For later reference we explicitly state the following simple Corollary.
\begin{lem}
...
\marginpar{In order to prove the estimates below, we use the following
remarks: (?)}
...
B)$. We still have to check for validity of the estimates.\marginpar{\dots
to check that \dots(?)}
Clearly
\be{normnorm}
\begin{split}
...
\ref{lm:kbounds}.\marginpar{Par here ?}\\
