...
\documentclass[11pt,a4paper,draft,DIV11]{scrartcl}
...
We consider a system of $N$ bosons with repulsive interaction. This system is described on the Hilbert space $L^2_s(\Rbb^{3N})$ by the Hamiltonian
\bd
...
The function $f_N = 1 - w_N$ is the solution to the zero-energy scattering equation
\bd
\left(-\Delta + \frac{1}{2}V_N \right) f_N = 0
\ed
with boundary condition $\lim_{\lvert x\rvert \to \infty} f_N(x) = 1$. The scattering length $a_0$ of the potential $V$ is defined as
\bd
...
\bd
i \partial_t \ph = -\Delta \ph + \left(f_N V_N \ast \lvert \ph\rvert^2 \right) \ph,
\ed
...
\bd
\left\lvert \Tr\left(J \gamma_{N,t}^{(1)} \right) - \Tr\left( J \project{\ph}  \right) \right\rvert \leq \norm{J}_{HS} \frac{C}{N^{1/4}}e^{K_1 e^{K_2 t}}.
\ed
\end{thm}
...
\]
where $V_N(x) = N^2 V(Nx)$ with $V \in C_c^\infty(\R^3)$. We assume that $V$
is non-negative and spherically symmetric. By definition, the Hamiltonian
$\Hcal_N$ leaves sectors of $\fock$ with a fixed number of particles
invariant and $[\Hcal_N,\Ncal] = 0$.
...
\marginpar{I think this remark is out of place.}
A remark on notation: We use the symbol $C$ throughout the paper for constants which might differ in value, but which are always independent of $N$ and of time $t$.
...
\end{lem}
\begin{rem}
Notice that $c$ and $s$ are defined by series like those of the hyperbolic cosine respectively the hyperbolic sine. 
\end{rem}
  ...
  \begin{enumerate}
    ...
    \item \label{sup} Let $p$ and $s$ be the integral kernels defined in Lemma
      ...
      \[
        ...
        \norm{\varphi}_{H^2}.
      \]
\marginpar{I think all this comments should go in just one place if they go
at all.}
(Notice that in the whole paper, we use $\sup_x$ as a short-hand for $\operatorname{esssup}_{x\in \Rbb^3}$. In accordance with \eqr{no1}, our notation has to be read as $\norm{p_x}_{L^2} = \left( \int \di y\, \lvert p(y,x\rvert^2 \right)^{1/2}$.)
  \end{enumerate}
\end{lem}
...
\begin{prp} \label{p:wphi}
  ...
  \begin{align*}
    ...
    \|_{L^2}^2, \\
    \norm{N \nabla w_N(x-y)\varphi(x)\varphi(y)}_{L^2(\di x\di y)}^2 & =  \int dx dy \, N^2 |\nabla w_N(x-y)|^2 |\varphi(x)|^2 |\varphi(y)|^2 \le
    C N \| \varphi \|_{H^1}^2,
  \end{align*}
...
start just at the top of the next page.}
  ...
  0$. We give a proof of the formula on the subspace $\chi(\N \le M)
...
\begin{proof}[Proof of Lemma \ref{l:kernels}]
  ...
  inequality, Leibniz' rule, and triangle inequality. We begin with
  \[
  ...
  \]
  with \marginpar{I think the J-estimate is non-trivial? more details maybe? cf. estimate for \eqref{eq:kk1}.}
\marginpar{It comes directly from prop. \ref{p:wphi}}
  \[
  ...
  \ref{kr} From Lemma \ref{l:w}\ref{l:w1}, it follows that $w_N \leq 1$. Hence,
  \[
...
Define the time evolution of quantum fluctuations by
\bd
...
\end{split}
\ee
\marginpar{What is a phase? An operator? \dots where $b(N,t)$ is a scalar
function \dots I think this is the only phase dropped \dots More words about
this I think \dots}
with a phase $b(N,t)$ which, like all phases, will be dropped from now on without any further comment.

In \eqr{no3} we have made use of a cancellation in the term $\Ocal({N^{1/2}})$ (term which is linear in creation/annihilation operators) due to $\ph$ satisfying the modified Hartree equation
\bd
i\partial_t \ph = -\Delta \ph + \left(f_N N V_N \ast \lvert \ph \rvert^2 \right) \ph.
\ed
Compare \cite{RS2009}, but notice that due to the factor $f_N$ in the Hartree equation, in our case the cancellation is incomplete. This is essential for ensuring the correct coupling constant $8\pi a_0$ in the Gross-Pitaevskii equation and ensures further cancelation with the cubic terms (see below). This cancelation is revealed through the Bogoliubov transformation, to be calculated now.

We now identify cancellations between linear terms stemming from the transformation of linear terms and linear terms stemming from normal-ordering the transformation of cubic terms
\begin{align*}
...
& + \hc
\end{align*}
Between the term \eqr{linearterm} and \eqr{cubicterm}, after normal ordering and plugging in $c = \delta + p$ and $s = k + r$, we find that the linear term is cancelled up to two remainder terms (the $N^{1/2}$-term, line \eqr{linearterm}, is completely cancelled). Result:
\begin{align*}
...
\begin{rem}The cancellations enable us to give the necessary estimates for all terms, see chapter \ref{ch:generatorestimates}. If we had not identified the cancellation between the boxed terms, there would be contributions of order $N^1$.
\end{rem}
...
\end{lem}
\begin{proof}
We know that $\Lcal_N(t) = (i\partial_t T^\ast_t)T_t + T^\ast_t \Lcal_N^{(0)}(t)T_t$ and we have explictly calculated and bounded $T^\ast_t \Lcal_N^{(0)}(t)T_t$ in appendix \ref{ch:generatorestimates}. We now write only line \eqref{l9} and the first summand in line \eqref{l10} explicitly, all the other terms we collect in $\Lcal_{\textrm{rest}}$:
\begin{align*}
...
\ee
We now collect the estimates for lines \eqref{l7}, \eqref{l8} and \eqref{l10} through \eqref{l25} from appendix \ref{ch:generatorestimates} to obtain an upper bound for $\lvert\ev{\Lcal_{\textrm{rest}}}\rvert$, i.\,e.\ a lower bound for $-\lvert\ev{\Lcal_{\textrm{rest}}}\rvert$.
...
\end{lem}
\begin{proof}
We start by calculating $[T^\ast_t \Lcal_N^{(0)}(t)T_t,\Ncal]$. Summands which contain an equal number of annihilation and creation operators commute with $\Ncal$ and thus drop. The other summands get numerical prefactors, but except of that stay unchanged. (Because of these prefactors, the sign of the $+\hc$-part changes, but this is not important for our estimates.)
Now use the estimates from appendix \ref{ch:generatorestimates} (choose e.\,g.\ $\varepsilon = 1$) to bound all summands of the commutator $[T^\ast_t \Lcal_N^{(0)}(t)T_t,\Ncal]$.

The contribution $[(\partial_t T^\ast_t)T_t,\Ncal]$ is of the same form as $(\partial_t T^\ast_t)T_t$, only the particle-number conserving summands of the series drop out. Thus, the term can be estimated in the same way as $(\partial_t T^\ast_t)T_t$ in Lemma \ref{lm:timederivative}.
\end{proof}


\begin{prp}
\label{prp:Nbound}[Bound on Number of Fluctuations] \marginpar{to be doublechecked}
Let $\Psi \in \fock$ with $\scal{\Psi}{\left(\frac{\Ncal^2}{N} + \Ncal \right)\Psi} \leq C$ and $\scal{T^\ast_0 \Psi}{\Lcal_N(0)T^\ast_0 \Psi} \leq C$ for some constant $C$ independent of $N$. Then
\bd
...
\ed
whith fixed constants $C_1$ and $M_1$. Therefore
\bd
\tLcal := \Lcal_N(t) + C_1 e^{M_1 t}(\Ncal+1)
\ed
is a non-negative operator, and so it is sufficient to prove an upper bound for $u(t) := \scal{U_t T^\ast_0 \Psi}{(\tLcal+\Ncal+1)U_t T^\ast_0 \Psi}$. We want to apply Gronwall's Lemma, so we calculate the derivative:
\begin{align*}
...
\end{align*}
This expectation values can be bounded using Lemma \ref{lem:ldotbounds}, Lemma \ref{lem:lncommutatorbound} and Corollary \ref{c:reg1}:
\bd
u'(t) \leq C e^{M t} \scal{U_t T^\ast_0 \Psi}{\left(\frac{\Ncal^2}{N}+\Ncal+1+\Kcal+\tilV \right)U_t T^\ast_0 \Psi}.
\ed
By Lemma \ref{lm:kbounds} and Corollary \ref{cor:N2} we can eliminate $\Kcal$, $\tilV$ and $\Ncal^2/N$ at the cost of $\Lcal_N(t)$ appearing. Then, we add non-negative summands $\Ncal+1$ to produce a $\tLcal$ on the right hand side and arrive at
\bd
u'(t) \leq \tilde C e^{\tilde K t} u(t).
\ed
We now apply Gronwall's Lemma to conclude that
\bd
u(t) \leq u(0) \exp(C e^{Kt})
\ed
for some constants $C$ and $K$, both independent of $N$. The factor $u(0)$ is given by
\bd
u(0) = \scal{T^\ast_0 \Psi}{(\tLcalo + \Ncal+1)T^\ast_0 \Psi}.
\ed
By Proposition \ref{p:TNT}, we know that $\scal{T^\ast_0 \Psi}{(\Ncal+1)T^\ast_0 \Psi} \leq C\scal{\Psi}{(\Ncal+1)\Psi}$. Now by assumption we see that $u(0) \leq C$.
\end{proof}

\begin{lem}\marginpar{to be doublechecked}
\label{lem:tilV}
 There exists $C$ such that
\bd
\ev{\tilV} \leq C \ev{\Vcal_N} + C \norm{\ph}_{H^2}^2\ev{\frac{(\Ncal+1)^2}{N}}.
\ed
...
\[
  W = W_0 = W(\sqrt{N} \varphi) \qquad \text{and} \qquad T = T_0 = T(k_0).
\]
...
\begin{proof}[Proof of Lemma \ref{l:ap}]
  To simplify the notation we write $U_t := U_N(t)$.
...
\begin{cor}
\label{cor:N2}
\marginpar{I think this should go right under the lemma.}
...
\section{Time Evolution of Factorized States}
\begin{lem}
\label{lem:TrJ}
...
\marginpar{So I think it should be stated with a arbitrary function \dots}
\end{lem}
\begin{proof}
...
& = \frac{d_N^2}{N} \int \di x\di y \scal{W^\ast_t e^{-it\Hcal_N}P_N W_0 \Psi}{\left(a^\ast_y + \sqrt{N} \ph(y) \right)\left(a_x + \sqrt{N} \ph(x) \right) W^\ast_t e^{-it\Hcal_N t}W_0 \Psi} J(x,y) \nonumber\\
...
\end{align*}
\marginpar{This is ending to quick to my taste\dots}
In the last step, Proposition \ref{p:TNT} (\ref{TN2T}) has been used. Now we use the assumption on $\Psi$ and Corollary \ref{cor:N2} to finish the proof.
\end{proof}
...
\begin{split}
 ...
 & = 2\ev{\Kcal}+ 2\norm{\nabla_x p}^2 \ev{\Ncal} \leq C\ev{\Kcal + \Ncal}.
\end{split}
\end{equation}
...
\end{align*}
(The $\nabla$ in the second summand has to be understood with respect to the 'dot'-variable.)
...
\begin{itemize}
 ...
 \item $a^\#(s_x)$ and $a^\#(p_x)$ is always ok, and can easily be estimated using the $L^2$-Norm of $s_x$ respectively $p_x$
 \item pull out $\sup_x \norm{s_x}^2$, $\sup_x \norm{p_x}^2$ and $\norm{\ph}_\infty$ from the integrals, then $\int \di y\, NV_N(x-y) = b_0$ (independent of $x$) can be used
\item use Cauchy-Schwarz in such a way that at most two operators $a^\#$ act on each $\psi$. Usually, we also apply Hoelder's inequality in the form
\bd
\begin{split}
...
\end{split}
\ed
\item for terms containing $a^\# a^\# a^\#$ or $a^\# a^\# a^\# a^\#$ we need $\Ncal^2$, so a prefactor $1/N$ is necessary. 
\end{itemize}
...
We still have to give a bound for the $(\partial_t T^*_t)T_t$-part of the generator. We will expand $(\partial_t T^*_t)T_t$ as a series and bound the summands individually, usign the following lemma.
\begin{lem}
\label{lm:Bbound}
...
We now calculate and estimate the functions $f_i$ which appear in the series for $(\partial_t T^*_t)T_t$.
...
The operator $\partial_t\left((\partial_t T^*_t)T_t\right)$ can be bounded by applying $\partial_t$ to all summands of the series individually and then employing lemma \ref{lm:Bbound} and the following lemma. The second estimate is only needed for odd $n$.
\begin{lem}
\label{lm:deldelt}
...
\begin{cor}[Uniform $H^n$-regularity for the modified Hartree equation] \label{c:reg1}
ADD DETAILS
  \[
    \| \varphi_t \|_{H^1} \le B_1 \qquad \text{and} \qquad \| \varphi_t
    \|_{H^n} \le B_n e^{K_n t}.
...
\begin{cor}[Uniform $H^n$-regularity for the Gross-Pitaevskii equation] \label{c:reg2}
ADD DETAILS
  \[
    \| \varphi_t \|_{H^1} \le B_1 \qquad \text{and} \qquad \| \varphi_t
    \|_{H^n} \le B_n e^{K_n t}.
...
& \qquad\qquad\qquad\qquad \times N w_N(z-y) \cc{\ph(z)} \cc{\ph(y)} \di z \bigg\rvert^2 \nonumber \\
