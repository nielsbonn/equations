...
\label{s:intro}
We consider a system of $N$ identical bosons with repulsive interaction. In physically interesting situations typically $N$ is very large, of order $10^3$ up to $10^{50}$. On the Hilbert space $L^2_s(\Rbb^{3N})$ of wave functions symmetric w.\,r.\,t.\ permutation of the $N$ variables this system is described by the Hamiltonian
\bd
H_N = \sum_{j=1}^N (-\Delta_{x_j}) + \frac{1}{N}\sum_{i<j}^N N^3 V(N(x_i-x_j)),
\ed
where the sum in the interaction term runs over all pairs of particles.
...
\marginpar{Let $f_n$\dots}Let the function $f_N = 1 - w_N: \Rbb^3 \to \Rbb$ be the solution to the zero-energy scattering equation
\begin{equation}
\label{eq:scatt}
...
\ed
\marginpar{*}Notice that while the Schr\"odinger equation is linear, the Gross-Pitaevskii equation is a non-linear PDE. The advantage of the Gross-Pitaevskii equation is that it is a PDE in $L^2(\Rbb^3)$ while the Schr\"odinger equation is formulated in $L^2(\Rbb^{3N})$ for very large $N$.
...
%\]
\marginpar{given by the $\to$ with}The $k$-particle reduced density matrix associated with a state $\psi_{N,t} \in L^2_s(\Rbb^{3N})$ is the positive trace-class operator on $L^2(\Rbb^{3k})$ given by the integral kernel
\be{red_density}
\begin{split}
...
\end{split}
\ee
\marginpar{If\dots, it is obvious that\dots}The integration corresponds to taking the partial trace or `tracing out' $N-k$ particles. It is obvious that $\Tr \gamma_{N,t}^{(k)} = 1$ if $\norm{\psi_{N,t}} = 1$.

\marginpar{by $\to$ using}We denote the projection onto the state $\varphi_t \in L^2(\Rbb^3)$ by the Dirac bra-ket notation $\project{\varphi_t}$.

\marginpar{\dots which provides estimates for the\dots}We can now formulate the main theorem, which estimates the difference of the expectation values of one-particle observables w.\,r.\,t.\ the Schr\"odinger evolution and the effective evolution.
...
\ed
\item \marginpar{natural $\to$ convenient}In the second-quantized setting we are going to work in it is more natural to consider the time evolution of squeezed coherent states instead of factorized states. For squeezed coherent states a better rate of convergence can be obtained, see Theorem \ref{thm:main_squeezed}.
\marginpar{can be $\to$ is}
\end{enumerate}
\end{rems}
...
\marginpar{\dots equation with factorized initial data and Hamilt.
(formula), where $U$ satisfies $U^2 \le$, the one-particle reduced density
matrix converges\dots}
\[
...
is $8 \pi a_0$ and not $b_0$. %(Notice that the result in that paper is based on a compactness argument and does not give any information about the rate of convergence.)
...
\ee
\marginpar{we can $\to$ we}which has the correct limiting equation.
...
\begin{lem}[Zero-energy Scattering Equation \cite{ESY2010}] \label{l:w}
  ...
  with scaled potential $V_N$. Then the following hold:% with constants uniform in $N$.
  \begin{enumerate}
...
\begin{lem} \label{l:kernels}
  ...
  \]
  Then there exists a constant $C$%, uniform in $N$
...
\label{s:expectation}
Let $P_N$ be the projection onto the $N$-particle subspace of Fock space.
\begin{lem}
\label{lem:TrJ}
